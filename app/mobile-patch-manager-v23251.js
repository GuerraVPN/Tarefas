...
const APP_VERSION='2.3.25.1',APP_BUILD=273,APP_CHANNEL='alpha';
...
const MARK='__TAREFAS_PATCH_MANAGER_V273__';
...
async function renderHistory(box,installed){
  ...
  try{
    ...
    }).join(''):'<div class="tm-patch-empty">Ainda não há patches publicados para a Alpha 2.3.25.1.</div>';
...
;(()=>{
'use strict';
const MARK='__TAREFAS_ALPHA_AUTO_PATCH_CHECK_V273__';
if(globalThis[MARK])return;globalThis[MARK]=true;
...
globalThis.TarefasAlphaAutoPatchCheckV273=Object.freeze({
  version:'2.3.25.1',build:273,intervalMs:INTERVAL,run:autoCheckOfficialPatches
});
})();
