...
const BASE_VERSION='2.3.25.1';
const BUILD=273;
const SESSION_KEY='tarefasPushSession17';
const USER_KEY='usuarioLogado';
const ACTIVITY_KEY='sessao26_ultima_atividade';
const UNLOCK_KEY='tarefasSecureUnlocked23251';
const LOCK_KEY='tarefasSecureLockRequired23251';
...
const MARK='__TAREFAS_SECURE_ENTRY_23251__';
...
function css(){
  if(document.getElementById('tmSecureEntryCss23251'))return;
  const style=document.createElement('style');
  style.id='tmSecureEntryCss23251';
  style.textContent=`
    #tmSecureEntry23251{position:fixed;inset:0;z-index:2147483647;background:rgba(3,10,8,.96);backdrop-filter:blur(18px);display:flex;align-items:center;justify-content:center;padding:20px;font-family:inherit}
    #tmSecureEntry23251 *{box-sizing:border-box}
    #tmSecureEntry23251 .tm-sec-card{width:min(430px,100%);border:1px solid rgba(255,255,255,.12);background:#0d1714;border-radius:22px;padding:24px;box-shadow:0 25px 80px rgba(0,0,0,.48);color:#eefbf5}
    #tmSecureEntry23251 .tm-sec-icon{width:58px;height:58px;border-radius:18px;display:grid;place-items:center;background:#113d30;font-size:30px;margin-bottom:17px}
    #tmSecureEntry23251 h2{margin:0;font-size:23px;line-height:1.2;color:#fff}
    #tmSecureEntry23251 .tm-sec-who{margin:7px 0 2px;font-size:13px;color:#a9bdb5}
    #tmSecureEntry23251 .tm-sec-copy{margin:12px 0 18px;font-size:13px;line-height:1.5;color:#b8cbc4}
    #tmSecureEntry23251 .tm-sec-actions{display:grid;gap:9px}
    #tmSecureEntry23251 button{width:100%;border:0;border-radius:12px;padding:13px 14px;font:inherit;font-weight:850;cursor:pointer}
    #tmSecureEntry23251 .tm-sec-bio{background:#10b981;color:#05251a}
    #tmSecureEntry23251 .tm-sec-password-toggle{background:#1d2c27;color:#eaf7f1;border:1px solid rgba(255,255,255,.1)}
    #tmSecureEntry23251 button:disabled{opacity:.55;cursor:wait}
    #tmSecureEntry23251 .tm-sec-password{display:none;margin-top:12px;padding-top:14px;border-top:1px solid rgba(255,255,255,.09)}
    #tmSecureEntry23251 .tm-sec-password.open{display:block}
    #tmSecureEntry23251 label{display:block;font-size:12px;font-weight:800;color:#cfe0d9;margin-bottom:7px}
    #tmSecureEntry23251 input{width:100%;border:1px solid #365248;background:#09110f;color:#fff;border-radius:12px;padding:13px 12px;font:inherit;outline:none}
    #tmSecureEntry23251 input:focus{border-color:#10b981;box-shadow:0 0 0 3px rgba(16,185,129,.13)}
    #tmSecureEntry23251 .tm-sec-submit{margin-top:9px;background:#e8f5ef;color:#09251b}
    #tmSecureEntry23251 .tm-sec-error{min-height:18px;margin:10px 0 0;font-size:12px;line-height:1.4;color:#fca5a5}
    #tmSecureEntry23251 .tm-sec-note{margin:13px 0 0;text-align:center;font-size:10px;line-height:1.45;color:#7f958c}
  `;
...
async function passwordUnlock(password,lockId=currentLockId){
  ...
  try{
    ...
    const input=overlay?.querySelector('#tmSecurePassword23251');
...
async function showLock({autoBiometric=true}={}){
  ...
  overlay.innerHTML=`
    <div class="tm-sec-card">
      ...
      <form class="tm-sec-password" autocomplete="off">
        <label for="tmSecurePassword23251">Senha da conta</label>
        <input id="tmSecurePassword23251" type="password" autocomplete="current-password" inputmode="text" enterkeyhint="done">
  ...
  passwordToggle.addEventListener('click',()=>{
    ...
    setTimeout(()=>overlay?.querySelector('#tmSecurePassword23251')?.focus(),30);
  });

  form.addEventListener('submit',event=>{
    event.preventDefault();
    passwordUnlock(overlay?.querySelector('#tmSecurePassword23251')?.value,lockId);
  });
...
globalThis.TarefasSecureEntry23251=Object.freeze({
...
const MARK='__TAREFAS_DOM_LIFECYCLE_FIX_23251__';
if(globalThis[MARK])return;
globalThis[MARK]=true;
...
function installErrorStoreSanitizer(){
  ...
  if(!globalThis.__TAREFAS_ALPHA_ERROR_STORE_GUARD_23251__){
    globalThis.__TAREFAS_ALPHA_ERROR_STORE_GUARD_23251__=true;
  ...
  if(alpha?.getState&&!alpha.__domErrorFilter23251){
    const original=alpha.getState.bind(alpha);
    alpha.getState=()=>{
      const state=original();
      return sanitizeAlphaState(state);
    };
    alpha.__domErrorFilter23251=true;
  }

  window.dispatchEvent(new CustomEvent('tarefas:alpha-state',{detail:{source:'patch-2.3.25.1'}}));
}
...
function installDomLifecycleGuard(){
  ...
  if(globalThis.__TAREFAS_GET_ELEMENT_GUARD_23251__)return;
  globalThis.__TAREFAS_GET_ELEMENT_GUARD_23251__=true;
...
globalThis.TarefasDomLifecycleFix23251=Object.freeze({
  version:'2.3.25.1',
  page,
...
const MARK='__TAREFAS_FAVORITES_FILTERS_23251__';
if(globalThis[MARK])return;
globalThis[MARK]=true;
...
const CATALOG_KEY='tarefas_destinations_23251_'+String(me.id);
const RESTORE_KEY='tarefas_restore_state_23251';
...
function captureUiState(){
  ...
  document.querySelectorAll('input,select,textarea').forEach(el=>{
    ...
    if(el.closest('#tmPatchManager,#tmFavPicker23251,#tmSecureEntry23242,#tmSecureEntry23251'))return;
...
function ensureModalCss(){
  if(document.getElementById('tmFavPickerCss23251'))return;
  const s=document.createElement('style');s.id='tmFavPickerCss23251';s.textContent=`
  #tmFavPicker23251{position:fixed;inset:0;z-index:2147483000;background:rgba(2,6,23,.72);display:flex;align-items:flex-end;justify-content:center;padding-top:40px}
  #tmFavPicker23251 .tmfp-card{width:min(760px,100%);max-height:88vh;background:var(--v4-surface,#0b1114);color:inherit;border-radius:20px 20px 0 0;border:1px solid var(--v4-border,#243244);display:flex;flex-direction:column;overflow:hidden}
  #tmFavPicker23251 header{padding:14px 16px;display:flex;justify-content:space-between;gap:12px;align-items:center;border-bottom:1px solid var(--v4-border,#243244)}
  #tmFavPicker23251 header h3{margin:0;font-size:17px}
  #tmFavPicker23251 header button{border:0;background:transparent;color:inherit;font-size:20px}
  #tmFavPicker23251 .tmfp-body{overflow:auto;padding:10px 14px}
  #tmFavPicker23251 .tmfp-group{margin:12px 0 6px;font-size:11px;font-weight:900;opacity:.7;text-transform:uppercase}
  #tmFavPicker23251 label{display:flex;gap:10px;align-items:flex-start;padding:10px;border:1px solid var(--v4-border,#243244);border-radius:12px;margin-bottom:7px}
  #tmFavPicker23251 label input{margin-top:3px;width:18px;height:18px}
  #tmFavPicker23251 label span{min-width:0}
  #tmFavPicker23251 label strong{display:block;font-size:13px}
  #tmFavPicker23251 label small{display:block;font-size:10px;opacity:.65;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  #tmFavPicker23251 footer{padding:12px 14px;border-top:1px solid var(--v4-border,#243244);display:flex;gap:8px}
  #tmFavPicker23251 footer button{flex:1;border:0;border-radius:11px;padding:12px;font-weight:900}
  #tmFavPicker23251 .primary{background:#2563eb;color:white}
  #tmFavPicker23251 .secondary{background:var(--v4-surface-3,#182230);color:inherit}
  .tm-filter-manage-23251{display:flex!important;gap:5px!important;flex-wrap:wrap!important;margin-left:6px}
  .tm-filter-manage-23251 button{font-size:10px!important;padding:7px 8px!important}
  `;document.head.appendChild(s);
}

function openFavoritePicker(){
  ensureModalCss();
  document.getElementById('tmFavPicker23251')?.remove();
  const state=getState();
  ...
  const modal=document.createElement('div');modal.id='tmFavPicker23251';
  let group='';
...
function enhanceCentral(){
  ...
  if(favButton&&!favButton.dataset.v23251){
    favButton.dataset.v23251='1';
  ...
  document.querySelectorAll('[data-open-filter]').forEach(open=>{
    ...
    if(!row||row.querySelector('.tm-filter-manage-23251'))return;
    const filter=findFilterByHref(open.dataset.openFilter);
    if(!filter)return;
    const box=document.createElement('div');box.className='tm-filter-manage-23251';
...
globalThis.TarefasFavoritesFilters23251=Object.freeze({
  version:'2.3.25.1',
...
const MARK='__TAREFAS_AI_SYNC_FILTERS_23251__';
if(globalThis[MARK])return;
globalThis[MARK]=true;

const VERSION='2.3.25.1';
...
function installAiFetchOptimizer(){
  if(globalThis.__TAREFAS_AI_FETCH_OPT_23251__||!globalThis.fetch)return;
  globalThis.__TAREFAS_AI_FETCH_OPT_23251__=true;
...
function installTypingTimer(){
  if(globalThis.__TAREFAS_AI_TIMER_23251__)return;
  globalThis.__TAREFAS_AI_TIMER_23251__=true;
...
function installQueueRecovery(){
  if(globalThis.__TAREFAS_QUEUE_RECOVERY_23251__)return;
  globalThis.__TAREFAS_QUEUE_RECOVERY_23251__=true;
...
function installAdvancedTaskFilters(){
  ...
  if(!actions||!section||document.getElementById('tmTaskStatusFilter23251'))return;

  const status=document.createElement('select');
  status.id='tmTaskStatusFilter23251';
  ...
  clear.type='button';
  clear.id='tmTaskClearFilters23251';
  ...
  // A 2.3.25.1.4 captura inputs/selects com ID, então este novo status
...
function installPreventiveUiApi(){
  ...
  if(globalThis.TarefasUiSafe23251)return;
  const get=id=>document.getElementById(id);
  globalThis.TarefasUiSafe23251=Object.freeze({
    get,
...
globalThis.TarefasPatch23251=Object.freeze({
  version:VERSION,
  aiOptimized:true,
  queueRecovery:true,
...
const MARK='__TAREFAS_VERSION_AUTO_SYNC_23251__';
if(globalThis[MARK])return;
globalThis[MARK]=true;

const VERSION='2.3.25.1';
const BASE_VERSION='2.3.25.1';
const BASE_BUILD=273;
const SESSION_KEY='tarefasPushSession17';
const SNAPSHOT_KEY='tarefasVersionSnapshot23251';
...
async function syncVersions({paint=true}={}){
  ...
  try{
    ...
    document.querySelectorAll('.tm-app-brand small').forEach(el=>{
      const web=el.textContent.match(/WEB\s*([0-9.]+)/i)?.[1]||'7.9.1';
...
function ensureCss(){
  if(document.getElementById('tmVersionSyncCss23251'))return;
  const s=document.createElement('style');
  s.id='tmVersionSyncCss23251';
  s.textContent=`
  .tm-version-sync-23251{margin:14px 0;padding:14px;border:1px solid var(--v4-border,#263244);border-radius:16px;background:var(--v4-surface,#0b1114);color:inherit}
  .tm-version-sync-23251 .tmvs-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:12px}
  .tm-version-sync-23251 .tmvs-head strong{display:block;font-size:15px}
  .tm-version-sync-23251 .tmvs-head small{display:block;font-size:10px;opacity:.65;margin-top:3px}
  .tm-version-sync-23251 .tmvs-badge{font-size:10px;font-weight:900;border-radius:999px;padding:5px 8px;background:var(--v4-surface-3,#162131)}
  .tm-version-sync-23251 .tmvs-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
  .tm-version-sync-23251 .tmvs-item{border:1px solid var(--v4-border,#263244);border-radius:12px;padding:10px;background:var(--v4-surface-2,#0f171b)}
  .tm-version-sync-23251 .tmvs-item small{display:block;font-size:9px;opacity:.62;margin-bottom:3px}
  .tm-version-sync-23251 .tmvs-item strong{display:block;font-size:13px}
  .tm-version-sync-23251 .tmvs-item span{display:block;font-size:10px;opacity:.72;margin-top:3px}
  .tm-version-sync-23251 .tmvs-new{color:#22c55e}
  .tm-version-sync-23251 .tmvs-foot{display:flex;justify-content:space-between;gap:8px;align-items:center;margin-top:10px;font-size:10px;opacity:.7}
  .tm-version-sync-23251 button{border:1px solid var(--v4-border,#263244);background:var(--v4-surface-3,#162131);color:inherit;border-radius:9px;padding:7px 9px;font-size:10px;font-weight:850}
  @media(max-width:500px){.tm-version-sync-23251 .tmvs-grid{grid-template-columns:1fr 1fr}}
  `;
  document.head.appendChild(s);
}
...
function renderSnapshot(snapshot=readSnapshot()){
  ...
  let box=document.getElementById('tmVersionSync23251');
  if(!box){
    box=document.createElement('section');
    box.id='tmVersionSync23251';
    box.className='tm-version-sync-23251';
...
function installUiWatcher(){
  ...
  let syncBoxSeen=!!document.getElementById('tmVersionSync23251');
  let scheduled=false;

  const refreshIfNeeded=()=>{
    scheduled=false;
    const rootNow=!!document.getElementById('tmAppUpdates');
    const boxNow=!!document.getElementById('tmVersionSync23251');
    ...
    rootSeen=rootNow;
    syncBoxSeen=!!document.getElementById('tmVersionSync23251');
  };
  ...
  const obs=new MutationObserver(mutations=>{
    for(const m of mutations){
      ...
      if(m.target instanceof Element && m.target.closest?.('#tmVersionSync23251'))continue;
      ...
      for(const node of [...m.addedNodes,...m.removedNodes]){
        ...
        if(
          ...
          node.id==='tmVersionSync23251' ||
          node.querySelector?.('#tmAppUpdates,#tmVersionSync23251')
        ){
          relevant=true;
          break;
        }
...
function installAutoSync(){
  ...
  globalThis.TarefasVersionAutoSync23251=Object.freeze({
    version:VERSION,
    sync:run,
...
;(()=>{
'use strict';
const MARK='__TAREFAS_PRERELEASE_HARDENING_23251__';
...
function installVersionMuteGuard(){
  ...
  if(!api?.notify||api.__v23251MuteWrapped)return;
...
function normalizeSavedFilterScopes(){
  ...
  if(changed){s.updatedAt=Date.now();localStorage.setItem(k,JSON.stringify(s));window.dispatchEvent(new CustomEvent('tarefas:alpha-state',{detail:{source:'patch-2.3.25.1-filter-scope'}}))}
  return changed;
}

function installLifecycleToken(){
  if(globalThis.TarefasPageLifecycle23251)return;
  ...
  globalThis.TarefasPageLifecycle23251=Object.freeze({
    version:VERSION,
    token:()=>generation,
...
function diagnostics(){
  ...
  const snapshot=globalThis.TarefasVersionAutoSync23251?.snapshot?.()||null;
  return{
    version:VERSION,
    domGuard:!!globalThis.__TAREFAS_GET_ELEMENT_GUARD_23251__,
    ...
    versionSync:{available:!!globalThis.TarefasVersionAutoSync23251,lastSync:snapshot?.syncedAt||null}
  };
}
...
function boot(){
  ...
  globalThis.TarefasPreRelease23251=Object.freeze({version:VERSION,diagnostics,versionsMuted});
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
})();
;(()=>{
'use strict';
const MARK='__TAREFAS_BIOMETRIC_COLD_START_23251__';
if(globalThis[MARK])return;globalThis[MARK]=true;
globalThis.TarefasBiometricColdStart23251=Object.freeze({
  version:'2.3.25.1',
...
;globalThis.__TAREFAS_BETA_23251_CONSOLIDATED__=Object.freeze({version:'2.3.25.1',build:273,channel:'beta',consolidates:['2.3.24.1','2.3.24.2','2.3.24.3','2.3.24.4','2.3.24.5','2.3.24.6','2.3.24.7','2.3.24.8','2.3.24.9'],alphaAutoPatchCheck:true});

;globalThis.__TAREFAS_ALPHA_23251_WEB_BASE__=Object.freeze({version:'2.3.25.1',build:273,channel:'alpha',webVersion:'7.9.1',basedOn:'2.3.25'});
