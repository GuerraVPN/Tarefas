(()=>{
'use strict';
if(window.__TAREFAS_ORC_LOADER_V241__)return;
window.__TAREFAS_ORC_LOADER_V241__=true;
...
function load(src){return new Promise((resolve,reject)=>{if(document.querySelector(`script[data-orc241-src="${src}"]`))return resolve();const s=document.createElement('script');s.src=`${src}?v=2.3.11-b241`;s.defer=true;s.async=false;s.dataset.orc241Src=src;s.onload=resolve;s.onerror=()=>reject(new Error('Falha ao carregar '+src));document.body.appendChild(s)})}
(async()=>{try{for(const f of files)await load(f);document.documentElement.dataset.orc241Loaded=mod}catch(e){console.error('[ORC 241 loader]',e);const el=document.getElementById('rRecentList')||document.getElementById('guiaList')||document.getElementById('pedidoList')||document.getElementById('movList')||document.getElementById('cargaRefList')||document.getElementById('passagemList');if(el)el.innerHTML='<div class="orc-empty">Falha ao abrir este módulo. Atualize a tela e tente novamente.</div>'}})();
})();
