...
data class Session(val accessToken:String,val refreshToken:String,val email:String,val role:String,val activationRequired:Boolean)
...
data class ClientHome(val fullName:String,val email:String,val phone:String,val accountStatus:String,val limitAmount:Double,val riskTier:String?,val monthlyRate:Double?,val maxInstallments:Int,val usedLimit:Double,val loans:List<LoanSummary>)
...
class SessionStore(context:Context){
    ...
    fun saveSession(s:Session){p.edit().putString("access_token",s.accessToken).putString("refresh_token",s.refreshToken).putString("email",s.email).putString("role",s.role).putBoolean("activation_required",s.activationRequired).apply()}
    fun session():Session?{val a=p.getString("access_token",null)?:return null;return Session(a,p.getString("refresh_token","")?:"",p.getString("email","")?:"",p.getString("role","client")?:"client",p.getBoolean("activation_required",false))}
    fun clearSession(){p.edit().remove("access_token").remove("refresh_token").remove("email").remove("role").remove("activation_required").apply()}
...
object SupabaseApi{
    ...
    private fun sessionFromJson(text:String):Session{val j=JSONObject(text);val access=j.getString("access_token");val refresh=j.optString("refresh_token");val user=j.optJSONObject("user")?:JSONObject();val meta=user.optJSONObject("app_metadata")?:jwtPayload(access).optJSONObject("app_metadata")?:JSONObject();return Session(access,refresh,user.optString("email").ifBlank{jwtPayload(access).optString("email")},meta.optString("role","client"),meta.optBoolean("activation_required",false))}
    ...
    suspend fun loadClientHome(access:String):ClientHome{
        val pa=JSONArray(request("GET","/rest/v1/profiles?select=full_name,email,phone,account_status&limit=1",accessToken=access));val p=if(pa.length()>0)pa.getJSONObject(0)else JSONObject()
        ...
        return ClientHome(p.optString("full_name","Cliente"),p.optString("email"),p.optString("phone"),p.optString("account_status","active"),l.optDouble("limit_amount",0.0),if(l.isNull("risk_tier"))null else l.optString("risk_tier"),if(l.isNull("monthly_rate"))null else l.optDouble("monthly_rate"),l.optInt("max_installments",1),used,loans)
    }
}
