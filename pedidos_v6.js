...
let pedidos=[],allItems=[],selected=null,selectedItems=[],history=[],pedidoAttachments=[];
...
async function renderRelatorio(){
 try{
   const [g,m,dc,pc,det]=await Promise.all([
     supabaseClient.from('guias_orcamentarias').select('id,numero,status,situacao_fiscalizacao,etapa_orcamentaria,atualizado_em'),
     ...
     supabaseClient.from('orc_documentos_carga').select('tipo_referencia,referencia,versao'),
     supabaseClient.from('orc_passagens_carga').select('id,status,dependencia'),
     supabaseClient.from('orc_detentores_carga').select('dependencia')
   ]);
   const guias=g.error?[]:(g.data||[]);
   const movs=m.error?[]:(m.data||[]);
   const docsCarga=dc.error?[]:(dc.data||[]);
   const passagens=pc.error?[]:(pc.data||[]);
   const detentores=det.error?[]:(det.data||[]);
   ...
   $('rGuiasPronto').textContent=guias.filter(x=>x.etapa_orcamentaria==='pronto').length;
   ...
   const latestKeys=new Set(docsCarga.map(x=>`${x.tipo_referencia}|${x.referencia}`));
   $('rCargaDocs').textContent=latestKeys.size;
   $('rCargaPendentes').textContent=Math.max(0,14-latestKeys.size);
   $('rPassagens').textContent=passagens.length;
   $('rPassagensAndamento').textContent=passagens.filter(x=>x.status==='em_andamento').length;
   $('rDetentores').textContent=detentores.length;
...
function switchModule(mode){
 ...
 if($('materialCargaModule'))$('materialCargaModule').hidden=true;
 if($('passagemCargaModule'))$('passagemCargaModule').hidden=true;
...
function renderList(){
 ...
 for(const [d,list] of groups){
   ...
   h+=list.map(p=>{
     ...
     const consumo=p.tipo==='desrelacionamento_baixa'&&p.categoria==='consumo';
     return `<article class="pedido-card ${selected&&String(selected.id)===String(p.id)?'active':''}" data-pedido-id="${p.id}">
       ...
       <p>${esc(local)}${consumo?'':` · ${count} material(is)<br>Total: ${esc(money(p.valor_total))}`}</p>
     </article>`;
   }).join('');
 }
...
async function loadHistory(id){
 const [h,a]=await Promise.all([
   supabaseClient.from('pedido_orcamentario_tramitacoes').select('*').eq('pedido_id',id).order('criado_em',{ascending:false}).order('id',{ascending:false}),
   supabaseClient.from('pedido_orcamentario_anexos').select('*').eq('pedido_id',id).order('criado_em',{ascending:false}).order('id',{ascending:false})
 ]);
 if(h.error)throw h.error;if(a.error)throw a.error;
 history=h.data||[];pedidoAttachments=a.data||[];
}
...
function renderDetail(){
 ...
 const consumoSemItens=selected.tipo==='desrelacionamento_baixa'&&selected.categoria==='consumo';
 $('pdTotal').parentElement.hidden=consumoSemItens;
 $('pdItemsCount').closest('.pedido-sub').hidden=consumoSemItens;
 ...
 renderItems();renderPedidoAttachments();renderFlow();renderActions();renderHistory();
}
...
function renderPedidoAttachments(){
 $('pedidoAttachCount').textContent=`${pedidoAttachments.length} arquivo(s)`;
 $('pedidoAttachList').innerHTML=pedidoAttachments.length?pedidoAttachments.map(a=>`<div class="orc-note" style="display:flex;justify-content:space-between;gap:8px;align-items:center;margin-bottom:6px">
   <div><b>${esc(a.arquivo_nome)}</b><br><span style="font-size:8px;color:var(--v4-muted)">${dateTimeBR(a.criado_em)}${a.observacao?' · '+esc(a.observacao):''}</span></div>
   <div style="display:flex;gap:5px"><button type="button" class="orc-btn" data-ped-open="${a.id}">Abrir</button><button type="button" class="orc-btn" data-ped-down="${a.id}">Baixar</button></div>
 </div>`).join(''):'<div class="orc-empty">Nenhum arquivo anexado.</div>';
}
async function uploadPedidoFile(file,pedidoId,note){
 if(file.size>25*1024*1024)throw new Error(`${file.name}: arquivo maior que 25 MB.`);
 const path=`pedidos/${pedidoId}/${Date.now()}_${Math.random().toString(36).slice(2,7)}_${safeName(file.name)}`;
 const up=await supabaseClient.storage.from('pedidos-orcamentarios').upload(path,file,{cacheControl:'3600',upsert:false,contentType:file.type||undefined});
 if(up.error)throw up.error;
 const pub=supabaseClient.storage.from('pedidos-orcamentarios').getPublicUrl(path);
 const ins=await supabaseClient.from('pedido_orcamentario_anexos').insert([{
   pedido_id:pedidoId,arquivo_nome:file.name,arquivo_path:path,arquivo_url:pub.data.publicUrl,
 ...
 if(ins.error){await supabaseClient.storage.from('pedidos-orcamentarios').remove([path]);throw ins.error}
 await supabaseClient.from('pedido_orcamentario_tramitacoes').insert([{
   pedido_id:pedidoId,evento:'arquivo_adicionado',mensagem:`Arquivo anexado: ${file.name}`,
   usuario_id:String(user.id),perfil_id:profileId()
 }]);
}
async function addPedidoFiles(){
 if(!selected)return;
 const files=[...$('pedidoAddFiles').files];if(!files.length)return alert('Selecione um ou mais arquivos.');
 try{
   const note=$('pedidoAttachNote').value.trim();
   for(const f of files)await uploadPedidoFile(f,selected.id,note);
   $('pedidoAddFiles').value='';$('pedidoAttachNote').value='';
   toast('Arquivo(s) anexado(s).');await loadHistory(selected.id);renderPedidoAttachments();renderHistory();
...
function eventLabel(e){
 return ({
  ...
  arquivo_adicionado:'Arquivo anexado'
 }[e]||e);
}
...
function syncPedidoLocation(){
 ...
 const consumo=!dist&&cat==='consumo';
 const editor=$('pedidoItemsEditor')?.closest('.pedido-items-editor');
 if(editor)editor.hidden=consumo;
 if(consumo)$('pedidoItemsEditor').innerHTML='';
 else if(!$('pedidoItemsEditor').children.length)addEditorRow();
}
...
function openNewPedido(){
 ...
 $('pCategoria').value=category;syncPedidoLocation();
...
function openEditPedido(){
 ...
 syncPedidoLocation();
 const consumoEdit=selected.tipo==='desrelacionamento_baixa'&&selected.categoria==='consumo';
 if(!consumoEdit){
   $('pedidoItemsEditor').innerHTML='';
...
async function deletePedido(){
 ...
 if(paths.length)try{await supabaseClient.storage.from('pedidos-orcamentarios').remove(paths)}catch(_){}
 selected=null;selectedItems=[];history=[];pedidoAttachments=[];
...
async function createPedido(e){
 ...
 const consumo=!dist&&cat==='consumo';
 const items=consumo?[]:readEditorItems();
 if(!consumo&&!items.length)return alert('Adicione pelo menos um material.');
 if(!consumo&&items.some(i=>!i.nome||!i.numero_ficha||!(i.quantidade>0)||i.valor_unitario<0))return alert('Confira nome, número da ficha, quantidade e valor unitário dos materiais.');
 ...
 try{
   ...
   const initialFiles=[...$('pFiles').files];
   for(const f of initialFiles)await uploadPedidoFile(f,created.id,'Arquivo anexado no cadastro do pedido.');
   $('newPedidoBg').classList.remove('open');toast(consumo?'Pedido de Consumo cadastrado.':'Pedido cadastrado com os materiais.');
...
function bind(){
 ...
 $('btnPedidoAddFiles').onclick=addPedidoFiles;
 $('pedidoAttachList').onclick=e=>{
   const o=e.target.closest('[data-ped-open]'),d=e.target.closest('[data-ped-down]');
   const id=o?.dataset.pedOpen||d?.dataset.pedDown;if(!id)return;
   const a=pedidoAttachments.find(x=>String(x.id)===String(id));if(!a)return;
   if(o)window.open(a.arquivo_url,'_blank','noopener');
   else{const l=document.createElement('a');l.href=a.arquivo_url;l.download=a.arquivo_nome;l.target='_blank';l.click()}
 };
}
async function start(){
 ...
 if(modulo==='movimentacao'||modulo==='material_carga'||modulo==='passagem_carga')return;
