(function(){
'use strict';
const ADITAMENTO_VERSION='7.3.2';
...
async function loadLogo(){
  try{
    const r=await fetch('brasao_exercito.png?v=7.3.2',{cache:'force-cache'});
...
async function buildPdf(context){
  ...
  for(const day of context.days){
    const needed=day.includeStandby?104:58;y=h.ensure(y,needed);
    h.center(`SERVIÇO PARA O DIA ${fullDate(day.date)} (${weekName(day.date)})`,y,9.6,true);y+=3;
    y=drawServiceTable(doc,context,day,y,false);
    y+=5;doc.setFont('times','bold');doc.setFontSize(9.5);doc.text('- PASSAGEM DE SERVIÇO ÀS 08:00h.',17,y);y+=9;
    if(day.includeStandby){
      y=h.ensure(y,51);h.center(`SOBREAVISO PARA O DIA ${fullDate(day.date)} (${weekName(day.date)})`,y,9.6,true);y+=3;
      y=drawServiceTable(doc,context,day,y,true);y+=8;
    }else y+=3;
  }
