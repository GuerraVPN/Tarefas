...
(async()=>{u=user();if(!u?.id)return location.href='index.html';await Promise.all([loadUsers(),loadNotifs()]);await loadMessages();const q=new URLSearchParams(location.search);const target=q.get('usuario')||q.get('conversa');if(target)await openConv(target);const tab=q.get('tab');if(tab==='mensagens'){document.querySelectorAll('[data-central-tab]').forEach(x=>x.classList.toggle('active',x.dataset.centralTab==='mensagens'));document.querySelector('[data-central-panel="mensagens"]')?.removeAttribute('hidden');document.querySelector('[data-central-panel="notificacoes"]')?.setAttribute('hidden','');}realtime()})().catch(e=>alert('Erro na Central: '+e.message));
})();
