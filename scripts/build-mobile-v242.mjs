...
const VERSION='2.4.2',BUILD=277,WEB_VERSION='7.9.1';

async function patch(rel,fn,{required=true}={}){
  const file=path.join(dist,rel),before=await readFile(file,'utf8'),after=fn(before);
  if(required&&after===before)throw new Error('2.4.2: alteração não aplicada em '+rel);
...
for(const name of await readdir(dist)){
  ...
  source=source.replaceAll('2.4.0-b274','2.4.2-b277');
...
await patch('mobile-patch-manager-v240.js',source=>source
  ...
  .replace("const MARK='__TAREFAS_PATCH_MANAGER_V274__';","const MARK='__TAREFAS_PATCH_MANAGER_V277__';")
  .replace("source:'beta-2.3.24'","source:'beta-2.4.2'"),{required:false});
...
await patch('mobile-bootstrap.js',source=>{
  let out=source
    ...
    .replaceAll('__TAREFAS_RELEASE_240_BOOT__','__TAREFAS_BETA_242_BOOT__')
    .replaceAll('__TAREFAS_PATCH_SYSTEM_V274__','__TAREFAS_PATCH_SYSTEM_V277__')
    .replaceAll("channel:'official'","channel:'beta'")
    .replaceAll("version:'2.4.0'","version:'"+VERSION+"'")
    .replaceAll('build:274','build:'+BUILD);
  out+="\n;globalThis.__TAREFAS_BETA_242_BOOT__={version:'"+VERSION+"',build:"+BUILD+",channel:'beta',basedOn:'2.4.0',webVersion:'"+WEB_VERSION+"',launcherIconBridge:true,patchChannelsFollowApkPreferences:true};\n";
  return out;
});

await patch('mobile-alpha-v23223-fix.js',source=>source
  .replace("const VERSION='2.4.0',BUILD=274,MARK='__TAREFAS_RELEASE_NAV_V274__';","const VERSION='"+VERSION+"',BUILD="+BUILD+",MARK='__TAREFAS_BETA_NAV_V277__';")
  .replaceAll('TAREFAS 2.4.0 Oficial','TAREFAS '+VERSION+' Beta')
  .replaceAll('TarefasRelease240','TarefasBeta242'),{required:false});
...
await patch('mobile-alpha-v23221.js',source=>source
  ...
  .replaceAll('RELEASE 240','BETA 242'),{required:false});

await patch('mobile-alpha-v23221-tabs.js',source=>source
  .replace("const MARK='__TAREFAS_RELEASE_TABS_V274__',VERSION='2.4.0',BUILD=274","const MARK='__TAREFAS_BETA_TABS_V277__',VERSION='"+VERSION+"',BUILD="+BUILD)
...
await patch('native-mobile.js',source=>source
  .replaceAll("version:'2.4.0'","version:'"+VERSION+"'")
  .replaceAll('build:274','build:'+BUILD)
  .replaceAll('"2.4.0"','"2.4.2"')
  .replaceAll("'2.4.0'","'2.4.2'"),{required:false});
...
await patch('mobile-release-v240.js',source=>{
  ...
  out+="\n;globalThis.__TAREFAS_BETA_242__=Object.freeze({version:'"+VERSION+"',build:"+BUILD+",channel:'beta',webVersion:'"+WEB_VERSION+"',launcherIconBridge:true,profileHomeIcon:true,patchChannelsFollowApkPreferences:true});\n";
