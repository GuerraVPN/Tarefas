...
await import(pathToFileURL(path.resolve('scripts/build-mobile-v2318.mjs')).href+'?v=2319');
...
for(const name of await readdir(dist)){
  ...
  s=s.replaceAll('2.3.18','2.3.19').replaceAll('b248','b249')
     .replaceAll('__TAREFAS_INSTALLER_NATIVE_V248__','__TAREFAS_INSTALLER_NATIVE_V249__')
     .replaceAll('__TAREFAS_INSTALLER_TEST_V248__','__TAREFAS_INSTALLER_CACHE_V249__');
...
const installer=`
    private File stageApk(Uri source) throws Exception {
        ...
        InputStream in = null;
        try {
            if ("file".equalsIgnoreCase(source.getScheme())) in = new FileInputStream(new File(source.getPath()));
            else in = resolver.openInputStream(source);
            if (in == null) throw new IllegalStateException("Android não conseguiu ler o APK baixado.");
            try (InputStream src = in; OutputStream out = new FileOutputStream(target, false)) {
                byte[] buffer = new byte[64 * 1024]; int read; long total = 0;
                while ((read = src.read(buffer)) != -1) { out.write(buffer,0,read); total += read; }
                out.flush();
                if (total <= 0) throw new IllegalStateException("APK baixado está vazio.");
            }
        } catch (Exception err) { target.delete(); throw err; }
        if (getContext().getPackageManager().getPackageArchiveInfo(target.getAbsolutePath(), PackageManager.GET_ACTIVITIES) == null) {
            target.delete(); throw new IllegalStateException("O arquivo baixado não foi reconhecido como APK válido.");
        }
        return target;
    }
    ...
    private void launchStagedInstaller(PluginCall call, Uri original) {
        new Thread(() -> {
            try {
                ...
                JSObject ret=new JSObject(); ret.put("opened",true); ret.put("validated",true); ret.put("stagedSize",staged.length()); call.resolve(ret);
            } catch(Exception err){ call.reject("Falha ao preparar APK para instalação: "+err.getMessage(),err); }
        }).start();
    }

    @PluginMethod
    public void installApk(PluginCall call) {
        String raw=call.getString("uri",""); if(raw==null||raw.trim().isEmpty()){call.reject("URI do APK ausente.");return;}
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.O&&!getContext().getPackageManager().canRequestPackageInstalls()){
            try{Intent settings=new Intent(Settings.ACTION_MANAGE_UNKNOWN_APP_SOURCES,Uri.parse("package:"+getContext().getPackageName()));startActivityForResult(call,settings,"installPermissionResultV249");}
            catch(Exception err){call.reject("Não foi possível abrir Permitir desta fonte.",err);}return;
        }
        launchStagedInstaller(call,Uri.parse(raw));
    }

    @ActivityCallback
    private void installPermissionResultV249(PluginCall call, ActivityResult result) {
        if(call==null)return;
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.O&&!getContext().getPackageManager().canRequestPackageInstalls()){call.reject("Permita instalar apps desta fonte para o TAREFAS.");return;}
        launchStagedInstaller(call,Uri.parse(call.getString("uri","")));
    }
`;
if(java.includes('public void installApk(PluginCall call)')){
  // remove the previous v247/v248 installer block appended before the class closing brace
  const start=java.indexOf('    private Uri normalizeInstallUri');
  if(start>=0){const end=java.lastIndexOf('\n}');java=java.slice(0,start)+java.slice(end);}
}
const idx=java.lastIndexOf('\n}'); if(idx<0)throw new Error('fechamento da classe ausente');
java=java.slice(0,idx)+installer+java.slice(idx);
await writeFile(javaPath,java,'utf8');

await appendFile(path.join(dist,'native-mobile.js'),'\n;globalThis.__TAREFAS_INSTALLER_CACHE_V249__=true;\n','utf8');
console.log('TAREFAS Android 2.3.19 build 249 BETA: APK validado e copiado ao cache privado antes do Package Installer.');
