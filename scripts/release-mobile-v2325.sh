...
node --check app/mobile-beta-v2325.js
node --check app/mobile-patch-manager-v2325.js
node --check scripts/build-mobile-v2325.mjs
node --check scripts/verify-mobile-v2325.mjs
node scripts/build-mobile-v2325.mjs
node scripts/verify-mobile-v2325.mjs dist
...
sed -i 's/versionCode 1/versionCode 272/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.3.25"/' android/app/build.gradle
...
assert int(e['versionCode']) == 272, e
assert str(e['versionName']) == '2.3.25', e
PY
...
unzip -p "$UNSIGNED" assets/public/mobile-patch-manager-v2325.js > "$RUNNER_TEMP/mobile-patch-manager-v2325.js"
unzip -p "$UNSIGNED" assets/public/mobile-beta-v2325.js > "$RUNNER_TEMP/mobile-beta-v2325.js"
unzip -p "$UNSIGNED" assets/public/mobile-updates-v181.js > "$RUNNER_TEMP/mobile-updates-v181.js"
grep -q '__TAREFAS_BETA_NAV_V272__' "$RUNNER_TEMP/mobile-alpha-v23223-fix.js"
grep -q '__TAREFAS_BETA_2325__' "$RUNNER_TEMP/mobile-bootstrap.js"
grep -q '__TAREFAS_PATCH_MANAGER_V272__' "$RUNNER_TEMP/mobile-patch-manager-v2325.js"
grep -q '__TAREFAS_ALPHA_AUTO_PATCH_CHECK_V272__' "$RUNNER_TEMP/mobile-patch-manager-v2325.js"
grep -q '__TAREFAS_BETA_2325_CONSOLIDATED__' "$RUNNER_TEMP/mobile-beta-v2325.js"
...
cp app/mobile-beta-v2325.js app/mobile-patch-manager-v2325.js app/release-v2325.txt "$RUNNER_TEMP/package/app/"
cp scripts/build-mobile-v2325.mjs scripts/verify-mobile-v2325.mjs "$RUNNER_TEMP/package/scripts/"
cp dist/BETA_2_3_25.json "$RUNNER_TEMP/package/manifest/"
...
git add downloads/TAREFAS-2.3.25*
if ! git diff --cached --quiet; then git commit -m 'release(android): TAREFAS 2.3.25 beta build 272'; git push origin HEAD:app/releases; fi
cd "$GITHUB_WORKSPACE"

URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.3.25.apk'
...
jq -n --arg version "$VERSION" --argjson build "$BUILD" --arg channel 'beta' --arg web_version '7.8.2' --arg title 'TAREFAS 2.3.25 Beta — Pré-release da 2.4' --arg url "$URL" --arg sha "$SHA" '{version:$version,build:$build,channel:$channel,web_version:$web_version,title:$title,changelog:["📦 Consolidados diretamente no APK os patches 2.3.24.1 até 2.3.24.9.","🔐 Biometria somente em novo início real do app; retorno do segundo plano não cobra novamente.","🩹 Gerenciador de Patches atualizado para a base 2.3.25 / build 272.","⚡ Usuários com Alpha ativado verificam automaticamente os patches oficiais; instalação continua manual.","🚫 Sem Alpha ativado, a consulta automática de patches Alpha não é executada.","⭐ Favoritos e filtros preservam páginas, subabas, módulos e estado da tela.","🤖 Assistente IA com contexto otimizado e indicador de tempo de resposta.","☁️ Fila offline com retomada automática ao recuperar conexão.","🛡️ Proteções extras contra DOM desmontado e respostas assíncronas obsoletas.","🔄 Sincronização automática de versão de APK mantida com segurança contra loops."],mandatory:false,download_url:$url,sha256:$sha}' > "$RUNNER_TEMP/release.json"
...
test "$CODE" = '200'
jq -e '.ok == true and .version == "2.3.25" and .build == 272 and .channel == "beta"' "$RUNNER_TEMP/result.json" >/dev/null
