...
node --check app/mobile-patch-manager-v23231.js
node --check scripts/build-mobile-v23231.mjs
node --check scripts/verify-mobile-v23231.mjs
node scripts/build-mobile-v23231.mjs
node scripts/verify-mobile-v23231.mjs dist
...
sed -i 's/versionCode 1/versionCode 270/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.3.23.1"/' android/app/build.gradle
...
assert int(e['versionCode']) == 270, e
assert str(e['versionName']) == '2.3.23.1', e
PY
unzip -p "$UNSIGNED" assets/public/mobile-alpha-v23223-fix.js | grep -q '__TAREFAS_ALPHA_NAV_V270__'
unzip -p "$UNSIGNED" assets/public/mobile-bootstrap.js | grep -q '__TAREFAS_VERSION_LABEL_V270__'
unzip -p "$UNSIGNED" assets/public/mobile-patch-manager-v23231.js | grep -q '__TAREFAS_PATCH_MANAGER_V270__'
unzip -p "$UNSIGNED" assets/public/mobile-updates-v181.js | grep -q "const APP_CHANNEL = 'alpha';"
...
cp app/mobile-alpha-v23223-fix.js app/mobile-patch-manager-v23231.js app/release-v23231.txt "$RUNNER_TEMP/package/app/"
cp scripts/build-mobile-v23231.mjs scripts/verify-mobile-v23231.mjs "$RUNNER_TEMP/package/scripts/"
cp dist/ALPHA_2_3_23_1.json "$RUNNER_TEMP/package/manifest/"
...
git add downloads/TAREFAS-2.3.23.1*
if ! git diff --cached --quiet; then git commit -m 'release(android): TAREFAS 2.3.23.1 alpha build 270'; git push origin HEAD:app/releases; fi
cd "$GITHUB_WORKSPACE"
URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.3.23.1.apk'
...
jq -n --arg version "$VERSION" --argjson build "$BUILD" --arg channel 'alpha' --arg web_version '7.8.6' --arg title 'TAREFAS 2.3.23.1 Alpha — Patch Manager .tpatch' --arg url "$URL" --arg sha "$SHA" '{version:$version,build:$build,channel:$channel,web_version:$web_version,title:$title,changelog:["🩹 Novo Gerenciador de patches .tpatch sem reinstalar o APK.","📥 Importação manual de patches pelo seletor de arquivos do Android.","🔐 Patches oficiais verificados por SHA-256 do catálogo e SHA-256 interno do payload.","↩️ Patches podem ser desativados/revertidos, reativados ou removidos.","🧪 Baseada diretamente na Beta 2.3.23, preservando suas correções.","🏷️ Versão 2.3.23.1 Alpha / build 270."],mandatory:false,download_url:$url,sha256:$sha}' > "$RUNNER_TEMP/release.json"
...
test "$CODE" = '200'
jq -e '.ok == true and .version == "2.3.23.1" and .build == 270 and .channel == "alpha"' "$RUNNER_TEMP/result.json" >/dev/null
