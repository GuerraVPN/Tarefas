...
node --check app/mobile-alpha-v23251.js
node --check app/mobile-patch-manager-v23251.js
node --check scripts/build-mobile-v23251.mjs
node --check scripts/verify-mobile-v23251.mjs
node scripts/build-mobile-v23251.mjs
node scripts/verify-mobile-v23251.mjs dist
...
sed -i 's/versionCode 1/versionCode 273/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.3.25.1"/' android/app/build.gradle
...
assert int(e['versionCode']) == 273, e
assert str(e['versionName']) == '2.3.25.1', e
PY
...
unzip -p "$UNSIGNED" assets/public/mobile-patch-manager-v23251.js > "$RUNNER_TEMP/pm.js"
unzip -p "$UNSIGNED" assets/public/mobile-alpha-v23251.js > "$RUNNER_TEMP/alpha.js"
...
grep -q '__TAREFAS_ALPHA_NAV_V273__' "$RUNNER_TEMP/nav.js"
grep -q '__TAREFAS_ALPHA_23251__' "$RUNNER_TEMP/bootstrap.js"
grep -q '__TAREFAS_PATCH_MANAGER_V273__' "$RUNNER_TEMP/pm.js"
grep -q '__TAREFAS_ALPHA_23251_WEB_BASE__' "$RUNNER_TEMP/alpha.js"
grep -q "const APP_CHANNEL = 'alpha';" "$RUNNER_TEMP/updates.js"
...
cp app/mobile-alpha-v23251.js app/mobile-patch-manager-v23251.js app/release-v23251.txt "$RUNNER_TEMP/package/app/"
cp scripts/build-mobile-v23251.mjs scripts/verify-mobile-v23251.mjs "$RUNNER_TEMP/package/scripts/"
cp dist/ALPHA_2_3_25_1.json "$RUNNER_TEMP/package/manifest/"
...
git add downloads/TAREFAS-2.3.25.1*
if ! git diff --cached --quiet; then git commit -m 'release(android): TAREFAS 2.3.25.1 alpha build 273'; git push origin HEAD:app/releases; fi
cd "$GITHUB_WORKSPACE"

URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.3.25.1.apk'
...
jq -n --arg version "$VERSION" --argjson build "$BUILD" --arg channel 'alpha' --arg web_version "$WEB_VERSION" --arg title 'TAREFAS 2.3.25.1 Alpha — Web 7.9.1' --arg url "$URL" --arg sha "$SHA" '{version:$version,build:$build,channel:$channel,web_version:$web_version,title:$title,changelog:["🌐 Base Web atualizada de 7.8.2 para 7.9.1.","🖥️ Painel SITE 7.9.1 incorporado ao APK.","🚪 EXIT USERS avisa usuários comuns e os redireciona ao Login; Admin permanece ativo.","🔄 Reiniciar SITE usa a nova tela reiniciar.html.","⏻ Desligar SITE usa a nova tela desligado.html até o Admin liberar o sistema.","⚡ Melhorias de performance da Web 7.9.1 com redução de pollings legados.","🧪 Base Android preservada na Beta 2.3.25, agora como Alpha 2.3.25.1 / build 273."],mandatory:false,download_url:$url,sha256:$sha}' > "$RUNNER_TEMP/release.json"
...
test "$CODE" = '200'
jq -e '.ok == true and .version == "2.3.25.1" and .build == 273 and .channel == "alpha"' "$RUNNER_TEMP/result.json" >/dev/null
