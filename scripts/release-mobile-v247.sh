...
# Recria a cadeia até a base validada 2.4.3, aplica 2.4.5 e então a camada Beta 2.4.7 pré-release.
git fetch origin beta-2.3.25
BASE_DIR="$RUNNER_TEMP/base-2325-v247"
...
sed -i 's/versionCode 1/versionCode 282/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.4.7"/' android/app/build.gradle
...
assert int(e['versionCode']) == 282, e
assert str(e['versionName']) == '2.4.7', e
PY

unzip -p "$UNSIGNED" assets/public/mobile-launcher-icon-v241.js > "$RUNNER_TEMP/launcher-v247.js"
...
cp app/mobile-launcher-icon-v241.js app/release-v247.txt "$RUNNER_TEMP/package/app/"
cp app/android/TarefasLauncherIconPlugin.java "$RUNNER_TEMP/package/app/"
cp scripts/build-mobile-v247.mjs scripts/verify-mobile-v247.mjs "$RUNNER_TEMP/package/scripts/"
...
RELEASES_DIR="$RUNNER_TEMP/tarefas-releases-v247"
...
git add downloads/TAREFAS-2.4.7*
if ! git diff --cached --quiet; then
  git commit -m 'release(android): TAREFAS 2.4.7 beta build 282'
  git push origin HEAD:app/releases
fi
cd "$GITHUB_WORKSPACE"

URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.4.7.apk'
...
test "$CODE" = '200'
jq -e '.ok == true and .version == "2.4.7" and .build == 282 and .channel == "beta"' "$RUNNER_TEMP/result.json" >/dev/null
