...
const checks={
 'mobile-bootstrap.js':["const APP_VERSION = '2.0.1'",'const APP_BUILD = 201',"const WEB_VERSION = '7.6.0'",'Jogos'],
 'mobile-preload.js':["tarefasAppVersion = '2.0.1'","tarefasAppBuild = '201'"],
 'mobile-v12.js':['2.0.1 • WEB 7.6.0'],
 'mobile-updates-v181.js':['2.0.1','APP_BUILD = 201',"APP_CHANNEL = 'beta'",'Baixar e instalar atualização'],
 'native-mobile.js':['2.0.1','tarefas:update-download-progress'],
 'games.html':['games.css?v=2.0.1','games.js?v=2.0.1','leaderboardList','26º PEL PE MEC'],
 'games.js':['APP 2.0.1','WEB 7.6.0','stage.dataset.game=key',"on(v.c,'click',()=>jump())","on(v.c,'click',()=>flap())",'v1_9_8_games_leaderboard','v1_9_8_start_game_run','v1_9_8_submit_game_score','AudioDirector','setPointerCapture'],
 'games.css':['Android 2.0.1 beta','overflow-y:auto!important','touch-action:pan-y!important','data-game="snake"','data-game="game2048"','.bn-fab::before','--stage-h:clamp(218px,58vw,272px)'],
...
if(await exists('games.html')&&await exists('dashboard.html')){
 const g=await text('games.html'),d=await text('dashboard.html'),re=/SUPABASE_ANON_KEY\s*=\s*['\"]([^'\"]+)['\"]/;
 const kg=g.match(re)?.[1],kd=d.match(re)?.[1];
 if(!kg||!kd||kg!==kd)errors.push('chave Supabase dos Jogos divergente do app');
}
...
console.log(`VERIFY 2.0.1 build 201 BETA / WEB 7.6.0: ${root}`);
console.log(`Tamanho raiz: ${total} bytes`);
if(errors.length){for(const e of errors)console.error('ERRO:',e);process.exit(1)}
console.log('OK: 2.0.1 beta validada com scroll Android, enquadramento compacto, controles sem seleção e placar acessível.');
