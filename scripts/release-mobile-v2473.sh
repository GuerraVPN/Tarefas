...
node --check scripts/build-mobile-v2473.mjs
...
node scripts/build-mobile-v2473.mjs
node scripts/verify-mobile-v2473.mjs dist
...
sed -i 's/versionCode 1/versionCode 285/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.4.7.3"/' android/app/build.gradle
...
<vector xmlns:android="http://schemas.android.com/apk/res/android"
    ...
    <path android:fillColor="#08285B" android:pathData="M0,0H108V108H0Z"/>
...
assert int(e['versionCode']) == 285, e
assert str(e['versionName']) == '2.4.7.3', e
PY

unzip -p "$UNSIGNED" assets/public/mobile-bootstrap.js > "$RUNNER_TEMP/bootstrap-v2473.js"
...
checks={
 'bootstrap':['__TAREFAS_ALPHA_2468_SERVICOS_HOTBAR_FIX__',"const APP_VERSION = '2.4.7.3';",'const APP_BUILD = 285;'],
 'pm':["APP_VERSION='2.4.7.3',APP_BUILD=285,APP_CHANNEL='alpha'"],
 'updates':["const APP_VERSION = '2.4.7.3';",'const APP_BUILD = 285;',"const APP_CHANNEL = 'alpha';"]
}
for f,needles in checks.items():
 p=t/({'bootstrap':'bootstrap-v2473.js','pm':'pm-v2473.js','updates':'updates-v2473.js'}[f])
 x=p.read_text()
 for n in needles:
  assert n in x,(f,n)
boot=(t/'bootstrap-v2473.js').read_text()
dash=(t/'dashboard-v2473.js').read_text()
...
print('APK CHECK OK: Alpha 2.4.7.3 com somente 2.4.6.8 e sem cartão Próximo Serviço')
PY
...
cp app/mobile-launcher-icon-v241.js app/release-v2473.txt "$RUNNER_TEMP/package/app/"
...
cp scripts/build-mobile-v246.mjs scripts/build-mobile-v2473.mjs scripts/verify-mobile-v2473.mjs "$RUNNER_TEMP/package/scripts/"
...
RELEASES_DIR="$RUNNER_TEMP/tarefas-releases-v2473"
...
git add downloads/TAREFAS-2.4.7.3*
if ! git diff --cached --quiet; then
  git commit -m 'release(android): TAREFAS 2.4.7.3 alpha build 285'
  git push origin HEAD:app/releases
fi
cd "$GITHUB_WORKSPACE"

URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.4.7.3.apk'
...
jq -n \
  ...
  --arg title 'TAREFAS 2.4.7.3 Alpha — remoção do cartão Próximo Serviço' \
  --arg url "$URL" \
  --arg sha "$SHA" \
  '{
    version:$version,
    ...
    title:$title,
    changelog:[
      "🧪 Alpha 2.4.7.3 para remover o cartão Próximo Serviço do Dashboard.",
...
test "$CODE" = '200'
jq -e '.ok == true and .version == "2.4.7.3" and .build == 285 and .channel == "alpha"' "$RUNNER_TEMP/result.json" >/dev/null
