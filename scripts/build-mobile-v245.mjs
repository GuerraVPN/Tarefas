...
for(const name of await readdir(dist)){
  ...
  source=source.replaceAll('2.4.5-b280','2.4.5-b280').replaceAll('2.4.5-b280','2.4.5-b280');
...
await patch('mobile-patch-manager-v240.js',source=>source
  ...
  .replace("__TAREFAS_PATCH_MANAGER_V280__","__TAREFAS_PATCH_MANAGER_V280__")
  .replaceAll("source:'beta-2.4.2'","source:'beta-2.4.5'"),{required:true});
...
await patch('mobile-bootstrap.js',source=>{
  let out=source
    ...
    .replaceAll('__TAREFAS_BETA_242_BOOT__','__TAREFAS_BETA_245_BOOT__')
    .replaceAll('__TAREFAS_PATCH_SYSTEM_V277__','__TAREFAS_PATCH_SYSTEM_V280__')
  ...
  out+="\n;globalThis.__TAREFAS_BETA_245_BOOT__={version:'"+VERSION+"',build:"+BUILD+",channel:'beta',basedOn:'2.4.3',webVersion:'"+WEB_VERSION+"',launcherIconBridge:true,patchChannelsFollowApkPreferences:true,drawerScalesOnly:true};\n";
  return out;
});

await patch('mobile-alpha-v23223-fix.js',source=>source
  .replace("const VERSION='2.4.5',BUILD=280,MARK='__TAREFAS_BETA_NAV_V280__';","const VERSION='"+VERSION+"',BUILD="+BUILD+",MARK='__TAREFAS_BETA_NAV_V278__';")
...
await patch('mobile-alpha-v23221.js',source=>source
  ...
  .replaceAll('BETA 242','BETA 245'),{required:false});

await patch('mobile-alpha-v23221-tabs.js',source=>source
  .replace("const MARK='__TAREFAS_BETA_TABS_V277__',VERSION='2.4.2',BUILD=277","const MARK='__TAREFAS_BETA_TABS_V278__',VERSION='"+VERSION+"',BUILD="+BUILD)
...
await patch('mobile-release-v240.js',source=>{
  let out=source
    ...
    .replaceAll("version:'2.4.2',build:277,channel:'beta'","version:'"+VERSION+"',build:"+BUILD+",channel:'beta'");
  out=out.replaceAll('__TAREFAS_BETA_242__','__TAREFAS_BETA_245__');
  out=out.replaceAll("version:'2.4.0',build:274","version:'"+VERSION+"',build:"+BUILD);
  out+="\n;globalThis.__TAREFAS_BETA_245__=Object.freeze({version:'"+VERSION+"',build:"+BUILD+",channel:'beta',webVersion:'"+WEB_VERSION+"',launcherIconBridge:true,profileHomeIcon:true,patchChannelsFollowApkPreferences:true,drawerScalesOnly:true});\n";
