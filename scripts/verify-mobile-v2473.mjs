...
const must=(x,m)=>{if(!x)throw new Error('2.4.7.3 verify: '+m)};
...
must(b.includes("const APP_VERSION = '2.4.7.3';")&&b.includes('const APP_BUILD = 285;'),'versão/build do bootstrap incorretos');
must(pm.includes("APP_VERSION='2.4.7.3',APP_BUILD=285,APP_CHANNEL='alpha'"),'Patch Manager alpha incorreto');
must(u.includes("const APP_VERSION = '2.4.7.3';")&&u.includes('const APP_BUILD = 285;')&&(u.includes("const APP_CHANNEL = 'alpha';")||u.includes("const APP_CHANNEL='alpha';")),'updates incorreto');
must(d.includes('function ensureCard(){ return null; }'),'cartão Próximo Serviço ainda pode ser criado no dashboard');
must(!d.includes('card=document.createElement(\'article\')'),'dashboard ainda contém criação do cartão');
for(const html of ['dashboard.html','index.html','pessoal.html']){try{const h=await readFile(path.join(dir,html),'utf8');must(h.includes('mobile-dashboard-v184.js?v=2.4.7.3-b285'),html+' sem cache-bust do dashboard')}catch(e){if(e.code!=='ENOENT')throw e}}
