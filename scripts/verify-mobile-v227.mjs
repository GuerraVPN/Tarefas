...
const required = [
  ...
  'mobile-ai-v227.js','v7_5_1_version.js','v7_7_0_material_carga.js','v7_7_2_scale_export.js',
...
console.log('CONFERÊNCIA 1/3 — Integridade e versão 2.2.7 / Web 7.7.2');
const markers={
 'mobile-bootstrap.js':["const APP_VERSION = '2.2.7'",'const APP_BUILD = 227',"const WEB_VERSION = '7.7.2'"],
 'mobile-preload.js':["tarefasAppVersion = '2.2.7'","tarefasAppBuild = '227'"],
 'mobile-v12.js':['2.2.7 • WEB 7.7.2'],
 'mobile-updates-v181.js':["const APP_VERSION = '2.2.7'",'const APP_BUILD = 227',"const APP_CHANNEL = 'beta'"],
 'mobile-login-v17.js':['v7_7_1_autenticar_usuario','session_token'],
 'native-mobile.js':['2.2.7','tarefas:update-download-progress'],
 ...
 'index.html':['v7_7_1_autenticar_usuario','mobile-login-v17.js','mobile-ai-v227.js']
};
...
for(const rel of htmlFiles){const html=await text(rel);for(const asset of ['mobile-preload.js','mobile.css','mobile-bootstrap.js','native-mobile.js','mobile-updates-v181.js','mobile-ai-v227.js'])if(!html.includes(asset))errors.push(`${rel}: injeção ausente: ${asset}`);if(html.includes('?v=2.2.6'))errors.push(`${rel}: cache da beta anterior no APK`)}
...
console.log('CONFERÊNCIA 2/3 — Assistente IA seguro e exclusivo do Android');
if(await exists('mobile-ai-v227.js')){
  const s=await text('mobile-ai-v227.js');
  for(const marker of [
    '__TAREFAS_ANDROID_227_AI__','tarefasPushSession17','/functions/v1/tarefas-ai',
    "'X-Tarefas-Session': session",'BETA 2.2.7 · somente leitura',
    "page() === 'index.html'",'.bn-panel.open .bn-close','MAX_LOCAL_MESSAGES = 20'
  ]) if(!s.includes(marker)) errors.push(`mobile-ai-v227.js: recurso ausente: ${marker}`);
  for(const forbidden of ['GEMINI_API_KEY','OPENAI_API_KEY','sk-proj-']) if(s.includes(forbidden)) errors.push(`mobile-ai-v227.js: segredo/referência proibida no cliente: ${forbidden}`);
}
const perf={
 'v6_2_mobile.js':['__TAREFAS_V227_LIGHT_LOADER__',"['games.html','about.html','orcamentarios.html']"],
...
console.log(`VERIFY 2.2.7 build 227 BETA / WEB 7.7.2: ${htmlFiles.length} telas, ${total} bytes.`);
if(errors.length){for(const e of errors)console.error('ERRO: '+e);process.exit(1)}
console.log('OK: 3/3 conferências concluídas; IA, segurança e regressões validadas.');
