...
const must=(ok,msg)=>{if(!ok)throw new Error('2.4.3.1 verify: '+msg)};
...
must(pm.includes("const APP_VERSION='2.4.3.1',APP_BUILD=279,APP_CHANNEL='alpha'")&&pm.includes('__TAREFAS_PATCH_MANAGER_V279__'),'Patch Manager 2.4.3.1/279 ausente');
...
must(pm.includes('v1_8_get_beta_updates')&&pm.includes('v2_3_21_alpha_context'),'preferências Alpha/Alpha ausentes');
must(bootstrap.includes("const APP_VERSION = '2.4.3.1';")&&bootstrap.includes('const APP_BUILD = 279;'),'bootstrap versão/build incorretos');
must(bootstrap.includes('__TAREFAS_ALPHA_2431_BOOT__'),'marker bootstrap ausente');
...
must(nav.includes("VERSION='2.4.3.1',BUILD=279")||nav.includes('__TAREFAS_ALPHA_NAV_V279__'),'navegação não consolidada');
must(tabs.includes("VERSION='2.4.3.1',BUILD=279")||tabs.includes('__TAREFAS_ALPHA_TABS_V279__'),'abas não consolidadas');
must(release.includes('__TAREFAS_ALPHA_2431__'),'runtime Alpha 2.4.3.1 ausente');
must(preload.includes("tarefasAppVersion = '2.4.3.1'")&&preload.includes("tarefasAppBuild = '279'"),'preload incorreto');
must(updates.includes("const APP_VERSION = '2.4.3.1';")&&updates.includes('const APP_BUILD = 279;')&&updates.includes("const APP_CHANNEL = 'alpha';"),'updates incorreto');
...
const manifest=JSON.parse(await read('ALPHA_2_4_3_1.json'));
must(manifest.version==='2.4.3.1'&&manifest.build===279&&manifest.channel==='alpha','manifesto inválido');
...
console.log('VERIFY 2.4.3.1 ALPHA OK: build 279 + patches consolidados + Escalas + SHA por bytes.');
