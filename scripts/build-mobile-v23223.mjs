...
const VERSION='2.3.22.4',BUILD=267;
...
for(const name of await readdir(dist)){
  ...
  source=source.replaceAll('2.3.22.1-b264','2.3.22.4-b267');
...
await patch('mobile-bootstrap.js',source=>{
  let out=source
    ...
    .replaceAll('Alpha 2.3.22.1','Alpha 2.3.22.4')
    .replaceAll('2.3.22.1 Alpha','2.3.22.4 Alpha');
  ...
  out+=`\n;globalThis.__TAREFAS_ALPHA_23224__={version:'${VERSION}',build:${BUILD},channel:'alpha',fixes:['topLevelSections','sectionRouting','alphaVersionLabel']};\n`;
  return out;
});
...
await appendFile(path.join(dist,'mobile-bootstrap.js'),`\n;globalThis.__TAREFAS_VERSION_LABEL_V267__='${VERSION} Alpha';\n`,'utf8');
await writeFile(path.join(dist,'ALPHA_2_3_22_4.json'),JSON.stringify({version:VERSION,build:BUILD,channel:'alpha',base:'2.3.22',generatedAt:new Date().toISOString(),fixes:{topLevelSections:true,notificationCentralSeparated:true,downloadsView:true,favoritesView:true,toolsView:true,messagesView:true,versionLabel:true}},null,2)+'\n','utf8');
console.log(`TAREFAS Android ${VERSION} build ${BUILD} ALPHA: Notificações removidas somente do Acesso rápido.`);
