...
const must=(ok,msg)=>{if(!ok)throw new Error(`2.3.22.4 verify: ${msg}`)};
...
must(fix.includes("VERSION='2.3.22.4',BUILD=267"),'patch de navegação com versão/build incorretos');
must(fix.includes('tm-alpha-main-tabs'),'abas principais não encontradas');
must(fix.includes("tabs.filter(([key])=>key!=='notificacoes')"),'Notificações ainda aparece no Acesso rápido');
...
must(bootstrap.includes("const APP_VERSION = '2.3.22.4';"),'APP_VERSION não é 2.3.22.4');
must(bootstrap.includes('const APP_BUILD = 267;'),'APP_BUILD não é 267');
...
must(tabs.includes("VERSION='2.3.22.4',BUILD=267"),'runtime das abas ainda aponta para versão antiga');
must(runtime.includes("VERSION='2.3.22.4',BUILD=267"),'runtime Alpha ainda aponta para versão antiga');
must(preload.includes("tarefasAppVersion = '2.3.22.4'"),'preload ainda mostra versão antiga');
must(updates.includes("const APP_VERSION = '2.3.22.4';"),'módulo de atualização ainda mostra versão antiga');
must(updates.includes("const APP_CHANNEL = 'alpha';"),'canal de atualização não é alpha');
const manifest=JSON.parse(await read('ALPHA_2_3_22_4.json'));
must(manifest.version==='2.3.22.4'&&manifest.build===267&&manifest.channel==='alpha','manifesto Alpha inválido');
console.log('VERIFY 2.3.22.4 OK: Notificações permanece em Comunicação e foi removida do Acesso rápido.');
