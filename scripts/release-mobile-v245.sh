...
node scripts/verify-mobile-v245.mjs dist
...
sed -i 's/versionCode 1/versionCode 280/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.4.5"/' android/app/build.gradle
...
cp app/mobile-launcher-icon-v241.js app/release-v245.txt "$RUNNER_TEMP/package/app/"
...
git add downloads/TAREFAS-2.4.5*
if ! git diff --cached --quiet; then
  git commit -m 'release(android): TAREFAS 2.4.5 beta build 280'
  git push origin HEAD:app/releases
fi
cd "$GITHUB_WORKSPACE"

URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.4.5.apk'
...
jq -n \
  ...
  --arg title 'TAREFAS 2.4.5 Beta — Base 2.4.3' \
  --arg url "$URL" \
  --arg sha "$SHA" \
  '{
    version:$version,
...
test "$CODE" = '200'
jq -e '.ok == true and .version == "2.4.5" and .build == 280 and .channel == "beta"' "$RUNNER_TEMP/result.json" >/dev/null
