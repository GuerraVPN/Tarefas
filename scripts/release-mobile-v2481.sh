...
node --check scripts/build-mobile-v2481.mjs
node --check scripts/verify-mobile-v246.mjs
node --check scripts/verify-mobile-v2481.mjs
...
node scripts/build-mobile-v2481.mjs
node scripts/verify-mobile-v2481.mjs dist
...
sed -i 's/versionCode 1/versionCode 291/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.4.8.1"/' android/app/build.gradle
...
assert int(e['versionCode']) == 291, e
assert str(e['versionName']) == '2.4.8.1', e
PY

unzip -p "$UNSIGNED" assets/public/mobile-bootstrap.js > "$RUNNER_TEMP/bootstrap-v2481.js"
unzip -p "$UNSIGNED" assets/public/mobile-login-v17.js > "$RUNNER_TEMP/login-v2481.js"
unzip -p "$UNSIGNED" assets/public/dashboard.html > "$RUNNER_TEMP/dashboard-v2481.html"
unzip -p "$UNSIGNED" assets/public/dashboard.js > "$RUNNER_TEMP/dashboard-js-v2481.js"
unzip -p "$UNSIGNED" assets/public/mobile-patch-manager-v240.js > "$RUNNER_TEMP/pm-v2481.js"
...
assert "const APP_VERSION = '2.4.8.1';" in boot and "const APP_BUILD = 291;" in boot
assert "dashboard.html?app=2.4.8.1" in boot and "dashboard.html?app=2.4.8.1" in login
...
cp app/mobile-launcher-icon-v241.js app/release-v2481.txt "$RUNNER_TEMP/package/app/"
...
true
cp scripts/build-mobile-v246.mjs scripts/build-mobile-v2481.mjs scripts/verify-mobile-v2481.mjs "$RUNNER_TEMP/package/scripts/"
cp dist/ALPHA_2_4_8_1.json "$RUNNER_TEMP/package/manifest/"
...
git add downloads/TAREFAS-2.4.8.1*
if ! git diff --cached --quiet; then
  git commit -m 'release(android): TAREFAS 2.4.8.1 alpha build 291'
  git push origin HEAD:app/releases
fi
cd "$GITHUB_WORKSPACE"

URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.4.8.1.apk'
...
jq -n \
  ...
  --arg title 'TAREFAS 2.4.8.1 Alpha — correção da aprovação da Distribuição' \
  --arg url "$URL" \
  --arg sha "$SHA" \
  '{
    version:$version,
    ...
    title:$title,
    changelog:[
      "🧪 Alpha 2.4.8.1 para corrigir a aprovação da Distribuição na Fiscalização",
      "🛠️ Corrige a referência ambígua \\"motivo\\" na função de movimentação da Distribuição.",
      ...
      "🧪 Patch Manager e auditoria do APK preservados."
    ],
    mandatory:false,
    download_url:$url,
    sha256:$sha
...
test "$CODE" = '200'
jq -e '.ok == true and .version == "2.4.8.1" and .build == 291 and .channel == "alpha"' "$RUNNER_TEMP/result.json" >/dev/null
