...
node scripts/verify-mobile-v2476.mjs dist
...
sed -i 's/versionCode 1/versionCode 288/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.4.7.6"/' android/app/build.gradle
...
assert str(e['versionName']) == '2.4.7.6', e
PY
...
assert "const APP_VERSION = '2.4.7.6';" in boot and "const APP_BUILD = 288;" in boot
assert "dashboard.html?app=2.4.7.6" in boot and "dashboard.html?app=2.4.7.6" in login
...
cp app/mobile-launcher-icon-v241.js app/release-v2476.txt "$RUNNER_TEMP/package/app/"
...
git add downloads/TAREFAS-2.4.7.6*
if ! git diff --cached --quiet; then
  git commit -m 'release(android): TAREFAS 2.4.7.6 alpha build 288'
  git push origin HEAD:app/releases
fi
cd "$GITHUB_WORKSPACE"

URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.4.7.6.apk'
...
jq -n \
  ...
  --arg title 'TAREFAS 2.4.7.6 Alpha — remoção definitiva do cartão Próximo Serviço' \
  --arg url "$URL" \
  --arg sha "$SHA" \
  '{
    version:$version,
    ...
    title:$title,
    changelog:[
      "🧪 Alpha 2.4.7.6 para remover o cartão Próximo Serviço do Dashboard.",
...
test "$CODE" = '200'
jq -e '.ok == true and .version == "2.4.7.6" and .build == 288 and .channel == "alpha"' "$RUNNER_TEMP/result.json" >/dev/null
