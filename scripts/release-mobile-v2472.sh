...
node --check scripts/build-mobile-v2472.mjs
...
node scripts/build-mobile-v2472.mjs
node scripts/verify-mobile-v2472.mjs dist
...
sed -i 's/versionCode 1/versionCode 284/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.4.7.2"/' android/app/build.gradle
...
<vector xmlns:android="http://schemas.android.com/apk/res/android"
    ...
    <path android:fillColor="#08284B" android:pathData="M0,0H108V108H0Z"/>
...
assert int(e['versionCode']) == 284, e
assert str(e['versionName']) == '2.4.7.2', e
PY
...
checks={
 'bootstrap':['__TAREFAS_ALPHA_2468_SERVICOS_HOTBAR_FIX__',"const APP_VERSION = '2.4.7.2';",'const APP_BUILD = 284;'],
 'pm':["APP_VERSION='2.4.7.2',APP_BUILD=284,APP_CHANNEL='alpha'"],
 'updates':["const APP_VERSION = '2.4.7.2';",'const APP_BUILD = 284;',"const APP_CHANNEL = 'alpha';"]
}
for f,needles in checks.items():
 p=t/({'bootstrap':'bootstrap-v2472.js','pm':'pm-v2472.js','updates':'updates-v2472.js'}[f])
 x=p.read_text()
 for n in needles:
  assert n in x,(f,n)
...
RELEASES_DIR="$RUNNER_TEMP/tarefas-releases-v2472"
...
git add downloads/TAREFAS-2.4.7.2*
if ! git diff --cached --quiet; then
  git commit -m 'release(android): TAREFAS 2.4.7.2 alpha build 284'
  git push origin HEAD:app/releases
fi
cd "$GITHUB_WORKSPACE"

URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.4.7.2.apk'
...
test "$CODE" = '200'
jq -e '.ok == true and .version == "2.4.7.2" and .build == 284 and .channel == "alpha"' "$RUNNER_TEMP/result.json" >/dev/null
