...
sed -i 's/versionCode 1/versionCode 289/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.4.7.7"/' android/app/build.gradle
...
git add downloads/TAREFAS-2.4.7.7*
if ! git diff --cached --quiet; then
  git commit -m 'release(android): TAREFAS 2.4.7.7 alpha build 289'
  git push origin HEAD:app/releases
fi
cd "$GITHUB_WORKSPACE"

URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.4.7.7.apk'
...
jq -n \
  ...
  --arg title 'TAREFAS 2.4.7.7 Alpha — remoção definitiva do cartão Próximo Serviço' \
  --arg url "$URL" \
  --arg sha "$SHA" \
  '{
    version:$version,
    ...
    title:$title,
    changelog:[
      "🧪 Alpha 2.4.7.7 para remover o cartão Próximo Serviço do Dashboard.",
