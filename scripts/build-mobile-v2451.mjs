...
const VERSION='2.4.5.1.1',BUILD=281,WEB_VERSION='7.9.1';
...
for(const name of await readdir(dist)){
  ...
  source=source.replaceAll('2.4.3-b278','2.4.5.1.1-b281');
  source=source.replaceAll('2.4.2-b277','2.4.5.1.1-b281');
  source=source.replaceAll('2.4.0-b274','2.4.5.1.1-b281');
...
await patch('mobile-patch-manager-v240.js',source=>source
  ...
  .replaceAll("source:'beta-2.4.3'","source:'beta-2.4.5.1'"),{required:true});
...
await patch('mobile-bootstrap.js',source=>{
  ...
  out=out.replace("__TAREFAS_ALPHA_2451_BOOT__={version:'"+VERSION+"',build:"+BUILD+",channel:'alpha',basedOn:'2.4.3'", "__TAREFAS_ALPHA_2451_BOOT__={version:'"+VERSION+"',build:"+BUILD+",channel:'alpha',basedOn:'2.4.3'");
  return out;
},{required:true});
...
await patch('mobile-alpha-v23221-tabs.js',source=>source
  .replace("const MARK='__TAREFAS_BETA_TABS_V278__',VERSION='2.4.3',BUILD=278","const MARK='__TAREFAS_BETA_TABS_V280__',VERSION='2.4.5.1',BUILD=281")
...
await patch('mobile-release-v240.js',source=>{
  ...
  out+="\n;globalThis.__TAREFAS_ALPHA_2451__=Object.freeze({version:'"+VERSION+"',build:"+BUILD+",channel:'alpha',basedOn:'2.4.3',webVersion:'"+WEB_VERSION+"',launcherIconBridge:true,profileHomeIcon:true,patchChannelsFollowApkPreferences:true,drawerScalesOnly:true,officialPatchSha256Bytes:true});\n";
...
await writeFile(path.join(dist,'ALPHA_2_4_5_1.json'),JSON.stringify({
  version:VERSION,build:BUILD,channel:'alpha',base:'2.4.3',webVersion:WEB_VERSION,generatedAt:new Date().toISOString(),
