...
const must=(ok,msg)=>{if(!ok)throw new Error('2.4.7 verify: '+msg)};
...
must(pm.includes("const APP_VERSION='2.4.7',APP_BUILD=282,APP_CHANNEL='beta'")&&pm.includes('__TAREFAS_PATCH_MANAGER_V282__'),'Patch Manager 2.4.7/282 ausente');
...
must(bootstrap.includes('__TAREFAS_ALPHA_2467_ESCALAS_2433__'),'patch 2.4.6.7 ausente');
must(bootstrap.includes('__TAREFAS_ALPHA_2468_SERVICOS_HOTBAR_FIX__'),'patch 2.4.6.8 ausente');
must(bootstrap.includes("scaleImplementation:'2.4.3.3'"),'Escalas não é a implementação 2.4.3.3');
must(bootstrap.includes('hotbarServicesRestored:true'),'Serviços 2.4.6.8 não restaurado');
