...
node --check app/mobile-launcher-icon-v2401.js
node --check scripts/build-mobile-v2401.mjs
node --check scripts/verify-mobile-v2401.mjs
node scripts/verify-web.mjs .

git fetch origin beta-2.3.25
BASE_DIR="$RUNNER_TEMP/base-2325-v2401"
...
node scripts/build-mobile-v2401.mjs
node scripts/verify-mobile-v2401.mjs dist
...
sed -i 's/versionCode 1/versionCode 275/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.4.0.1"/' android/app/build.gradle
...
assert int(e['versionCode']) == 275, e
assert str(e['versionName']) == '2.4.0.1', e
PY

unzip -p "$UNSIGNED" assets/public/mobile-launcher-icon-v2401.js > "$RUNNER_TEMP/launcher-v2401.js"
unzip -p "$UNSIGNED" assets/public/mobile-bootstrap.js > "$RUNNER_TEMP/bootstrap-v2401.js"
unzip -p "$UNSIGNED" assets/public/mobile-patch-manager-v240.js > "$RUNNER_TEMP/pm-v2401.js"
unzip -p "$UNSIGNED" assets/public/mobile-updates-v181.js > "$RUNNER_TEMP/updates-v2401.js"
grep -q '__TAREFAS_LAUNCHER_ICON_2401__' "$RUNNER_TEMP/launcher-v2401.js"
grep -q 'pinProfileShortcut' "$RUNNER_TEMP/launcher-v2401.js"
grep -q '__TAREFAS_ALPHA_2401_BOOT__' "$RUNNER_TEMP/bootstrap-v2401.js"
grep -q '__TAREFAS_PATCH_MANAGER_V275__' "$RUNNER_TEMP/pm-v2401.js"
grep -q "const APP_CHANNEL = 'alpha';" "$RUNNER_TEMP/updates-v2401.js"
...
cp app/mobile-launcher-icon-v2401.js app/release-v2401.txt "$RUNNER_TEMP/package/app/"
cp app/android/TarefasLauncherIconPlugin.java "$RUNNER_TEMP/package/app/"
cp scripts/build-mobile-v2401.mjs scripts/verify-mobile-v2401.mjs "$RUNNER_TEMP/package/scripts/"
cp dist/ALPHA_2_4_0_1.json "$RUNNER_TEMP/package/manifest/"
(cd "$RUNNER_TEMP/package" && zip -qr "$GITHUB_WORKSPACE/$ZIP" .)
unzip -tq "$ZIP"
sha256sum "$ZIP" | tee "$ZIP.sha256"

git fetch origin app/releases
RELEASES_DIR="$RUNNER_TEMP/tarefas-releases-v2401"
...
git add downloads/TAREFAS-2.4.0.1*
if ! git diff --cached --quiet; then
  git commit -m 'release(android): TAREFAS 2.4.0.1 alpha build 275'
  git push origin HEAD:app/releases
fi
cd "$GITHUB_WORKSPACE"

URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.4.0.1.apk'
...
jq -n \
  ...
  --arg title 'TAREFAS 2.4.0.1 Alpha — Seletor de ícones' \
  --arg url "$URL" \
  --arg sha "$SHA" \
  '{
    version:$version,
    ...
    title:$title,
    changelog:[
      "🎨 Novo seletor de ícones dentro das Configurações do app.",
      "🏠 O ícone escolhido pode ser aplicado ao atalho TAREFAS da tela inicial.",
      "👤 Novo modo Meu perfil: usa a imagem/ícone do perfil do usuário na tela inicial.",
      "🔵 Modelo TAREFAS Azul incluído e aplicado como ícone padrão corrigido do APK.",
      "🪖 Incluídos os modelos Militar, Preto & Ouro e Sistema.",
      "🔁 Depois de fixar o atalho uma vez, novas escolhas atualizam o mesmo atalho.",
      "🌐 Mantida a Base Web 7.9.1 e todas as funções da Oficial 2.4.0."
    ],
    mandatory:false,
    download_url:$url,
    sha256:$sha
...
test "$CODE" = '200'
jq -e '.ok == true and .version == "2.4.0.1" and .build == 275 and .channel == "alpha"' "$RUNNER_TEMP/result.json" >/dev/null
