...
const must=(x,m)=>{if(!x)throw new Error('2.4.7.6 verify: '+m)};
const patchFile=path.join(root,'patches','TAREFAS-2.4.6.8.tpatch');
await access(patchFile);
for(const f of ['mobile-login-v17.js','mobile-bootstrap.js','mobile-patch-manager-v240.js','mobile-updates-v181.js','mobile-release-v240.js','dashboard.html','dashboard.js','ALPHA_2_4_7_6.json'])await access(path.join(dir,f));
...
must(login.includes("dashboard.html?app=2.4.7.6"),'login não usa nova entrada');
must(b.includes("dashboard.html?app=2.4.7.6"),'hotbar não usa nova entrada');
...
must(m.version==='2.4.7.6'&&m.build===288&&m.channel==='alpha'&&m.base==='2.4.6'&&m.basedOn==='2.4.7.4'&&m.incorporatedPatch==='2.4.6.8','manifesto incorreto');
...
must(pm.includes("const APP_VERSION='2.4.7.6',APP_BUILD=288,APP_CHANNEL='alpha';"),'Patch Manager não foi preservado/atualizado');
...
console.log('VERIFY 2.4.7.6 ALPHA OK: caminho do Dashboard reestruturado; nenhum HTML carrega v184/v185; somente patch 2.4.6.8.');
