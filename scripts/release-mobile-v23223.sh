...
sed -i 's/versionCode 1/versionCode 269/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.3.23"/' android/app/build.gradle
...
assert int(e['versionCode']) == 269, e
assert str(e['versionName']) == '2.3.23', e
PY
unzip -p "$UNSIGNED" assets/public/mobile-alpha-v23223-fix.js | grep -q '__TAREFAS_BETA_NAV_V269__'
unzip -p "$UNSIGNED" assets/public/mobile-bootstrap.js | grep -q '__TAREFAS_VERSION_LABEL_V269__'
...
cp dist/BETA_2_3_23.json "$RUNNER_TEMP/package/manifest/"
