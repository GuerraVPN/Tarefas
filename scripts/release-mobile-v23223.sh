...
sed -i 's/versionCode 1/versionCode 268/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.3.22.5"/' android/app/build.gradle
...
assert int(e['versionCode']) == 268, e
assert str(e['versionName']) == '2.3.22.5', e
PY
unzip -p "$UNSIGNED" assets/public/mobile-alpha-v23223-fix.js | grep -q '__TAREFAS_ALPHA_NAV_V268__'
unzip -p "$UNSIGNED" assets/public/mobile-bootstrap.js | grep -q '__TAREFAS_VERSION_LABEL_V268__'
...
cp dist/ALPHA_2_3_22_5.json "$RUNNER_TEMP/package/manifest/"
...
git add downloads/TAREFAS-2.3.22.5*
if ! git diff --cached --quiet; then git commit -m 'release(android): TAREFAS 2.3.22.5 alpha build 268'; git push origin HEAD:app/releases; fi
cd "$GITHUB_WORKSPACE"
URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.3.22.5.apk'
...
jq -n --arg version "$VERSION" --argjson build "$BUILD" --arg channel 'alpha' --arg web_version '7.8.6' --arg title 'TAREFAS 2.3.22.5 Alpha — Acesso rápido removido' --arg url "$URL" --arg sha "$SHA" '{version:$version,build:$build,channel:$channel,web_version:$web_version,title:$title,changelog:["🧹 Removido por completo o bloco Acesso rápido do menu lateral.","📑 Notificações, Mensagens, Downloads, Favoritos e Ferramentas continuam disponíveis somente nas abas próprias.","✅ Nenhuma outra navegação ou função foi alterada.","🏷️ Versão 2.3.22.5 Alpha / build 268." ],mandatory:false,download_url:$url,sha256:$sha}' > "$RUNNER_TEMP/release.json"
...
test "$CODE" = '200'
jq -e '.ok == true and .version == "2.3.22.5" and .build == 268 and .channel == "alpha"' "$RUNNER_TEMP/result.json" >/dev/null
