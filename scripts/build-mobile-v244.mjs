...
const VERSION='2.4.4',BUILD=279,WEB_VERSION='7.9.1';

async function patch(rel,fn,{required=true}={}){
  const file=path.join(dist,rel),before=await readFile(file,'utf8'),after=fn(before);
  if(required&&after===before)throw new Error('2.4.4: alteração não aplicada em '+rel);
...
await patch('mobile-bootstrap.js',source=>{
  let out=source
    ...
    .replaceAll('__TAREFAS_PATCH_SYSTEM_V277__','__TAREFAS_PATCH_SYSTEM_V279__')
  ...
  if(!out.includes(oldClick))throw new Error('2.4.4: handler do drawer não encontrado');
  out=out.replace(oldClick,newClick);

  out+="\n;globalThis.__TAREFAS_BETA_244_BOOT__={version:'"+VERSION+"',build:"+BUILD+",channel:'beta',basedOn:'2.4.2',webVersion:'"+WEB_VERSION+"',launcherIconBridge:true,patchChannelsFollowApkPreferences:true,drawerScalesOnly:true};\n";
  return out;
});
...
await patch('mobile-alpha-v23221.js',source=>source
  ...
  .replaceAll('BETA 242','BETA 244'),{required:false});
...
await patch('mobile-release-v240.js',source=>{
  ...
  out=out.replaceAll('__TAREFAS_BETA_242__','__TAREFAS_BETA_244__');
  out=out.replaceAll("version:'2.4.0',build:274","version:'"+VERSION+"',build:"+BUILD);
  out+="\n;globalThis.__TAREFAS_BETA_244__=Object.freeze({version:'"+VERSION+"',build:"+BUILD+",channel:'beta',webVersion:'"+WEB_VERSION+"',launcherIconBridge:true,profileHomeIcon:true,patchChannelsFollowApkPreferences:true,drawerScalesOnly:true});\n";
