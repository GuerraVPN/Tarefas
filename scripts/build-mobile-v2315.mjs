...
// 2.3.15: volta ao armazenamento fixo planejado.
// - Atualizações: Documentos/TAREFAS/Oficial ou Documentos/TAREFAS/Beta
// - Arquivos gerados/baixados: Downloads/TAREFAS
// - Sem "Salvar como" e sem acesso total aos arquivos.
const nativePath = path.join(root, 'app', 'native-mobile-entry.js');
let native = await readFile(nativePath, 'utf8');

if (!native.includes("const UPDATES_FOLDER = `${FILES_FOLDER}/Atualização`;")) {
  throw new Error('native-mobile-entry.js: UPDATES_FOLDER antigo não encontrado');
}
native = native.replace(
  "const UPDATES_FOLDER = `${FILES_FOLDER}/Atualização`;",
  'const UPDATES_FOLDER = FILES_FOLDER;'
);
...
if (!permissionGate.test(native)) throw new Error('native-mobile-entry.js: gate de permissão legado não encontrado');
native = native.replace(permissionGate, '\n if(Capacitor.isNativePlatform()){');

// Abrir input de arquivo usa o seletor do próprio Android e não precisa solicitar armazenamento público.
native = native.replace(
...
// About: remove o fluxo de "Acesso a todos os arquivos" e mostra os diretórios fixos.
...
updates = updates.replace(
  oldState,
  "const storage={granted:true,required:false,fixedDirectories:true};return{beta:beta===true,latest:Array.isArray(latestRows)?latestRows[0]||null:latestRows||null,history:Array.isArray(history)?history:[],storage}"
);
...
updates = updates.replace(
  ...
  "    const storageText='Sem permissão especial: o TAREFAS usa somente as pastas fixas do próprio fluxo.';"
);
updates = updates.replace(
  'Documentos das tarefas são salvos em <b>Downloads/TAREFAS</b>. APKs de atualização continuam separados em Documentos/TAREFAS/Atualização/Beta ou Oficial.',
...
// Gera a base 2.3.14 (que neutraliza as APIs antigas de acesso total) já sobre estas fontes.
...
// O plugin que vai para o APK não contém mais ACTION_CREATE_DOCUMENT nem o método do picker.
