...
const required = [
  ...
  'v7_5_1_version.js','v7_7_0_material_carga.js','v7_7_2_scale_export.js','v7_7_2_site_patch.js',
  'v7_6_5_webfix.js','v6_2_mobile.js','v6_5_patch.js','v7_4_12_site.js','v7_4_12_global.js'
];
...
console.log('CONFERÊNCIA 1/3 — Integridade e versão 2.2.6 / Web 7.7.2');
const markers={
 'mobile-bootstrap.js':["const APP_VERSION = '2.2.6'",'const APP_BUILD = 226',"const WEB_VERSION = '7.7.2'"],
 'mobile-preload.js':["tarefasAppVersion = '2.2.6'","tarefasAppBuild = '226'"],
 'mobile-v12.js':['2.2.6 • WEB 7.7.2'],
 'mobile-updates-v181.js':["const APP_VERSION = '2.2.6'",'const APP_BUILD = 226',"const APP_CHANNEL = 'beta'"],
 'mobile-login-v17.js':['v7_7_1_autenticar_usuario','session_token'],
 'native-mobile.js':['2.2.6','tarefas:update-download-progress'],
 ...
 'v7_7_2_scale_export.js':['__TAREFAS_V772_SCALE_EXPORT__','application/vnd.oasis.opendocument.text','TAREFAS_SCALE_EXPORT_V772','TarefasNative?.files?.saveBlob'],
...
for(const rel of htmlFiles){const html=await text(rel);for(const asset of ['mobile-preload.js','mobile.css','mobile-bootstrap.js','native-mobile.js','mobile-updates-v181.js'])if(!html.includes(asset))errors.push(`${rel}: injeção ausente: ${asset}`);if(html.includes('?v=2.2.5')||html.includes('?v=7.7.1'))errors.push(`${rel}: cache da beta anterior no APK`)}
...
console.log('CONFERÊNCIA 2/3 — Exportação de escala e desempenho preservado');
if(await exists('v7_7_2_scale_export.js')){const s=await text('v7_7_2_scale_export.js');for(const marker of ["const service=page==='pessoal.html'", "mission=page==='missao.html'", "value=\"pdf\"", "value=\"odt\"", 'function odtBlob', 'function pdfBlob'])if(!s.includes(marker))errors.push(`v7_7_2_scale_export.js: recurso ausente: ${marker}`)}
const perf={
 'v6_2_mobile.js':['__TAREFAS_V226_LIGHT_LOADER__',"['games.html','about.html','orcamentarios.html']"],
...
console.log('CONFERÊNCIA 3/3 — Material Carga e regras existentes');
...
console.log(`VERIFY 2.2.6 build 226 BETA / WEB 7.7.2: ${htmlFiles.length} telas, ${total} bytes.`);
if(errors.length){for(const e of errors)console.error('ERRO: '+e);process.exit(1)}
console.log('OK: 3/3 conferências concluídas; ODT/PDF e regressões validados.');
