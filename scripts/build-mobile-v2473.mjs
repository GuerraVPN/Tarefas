...
const VERSION='2.4.7.3', BUILD=285, WEB_VERSION='7.9.1';
...
async function mustFile(file,label){try{await access(file)}catch{throw new Error('2.4.7.3: arquivo obrigatório ausente: '+label+' ('+file+')')}}
async function patch(rel,fn,{required=true}={}){const file=path.join(dist,rel);await mustFile(file,rel);const before=await readFile(file,'utf8');const after=fn(before);if(required&&after===before)throw new Error('2.4.7.3: alteração não aplicada em '+rel);if(after!==before)await writeFile(file,after,'utf8')}
...
if(patchData.id!==PATCH_ID||patchData.baseVersion!=='2.4.6')throw new Error('2.4.7.3: metadados do patch 2.4.6.8 inválidos');
if(!patchData.payload?.js)throw new Error('2.4.7.3: payload JS do patch 2.4.6.8 ausente');
const declaredSha=String(patchData.payloadSha256||'').toLowerCase();
if(declaredSha!==PATCH_SHA256)throw new Error('2.4.7.3: SHA-256 declarado do patch 2.4.6.8 não confere com o artefato oficial');
const payload=patchData.payload.js;
if(payload.includes('__TAREFAS_ALPHA_2467_ESCALAS_2433__'))throw new Error('2.4.7.3: payload proibido 2.4.6.7 detectado');
...
for(const name of ['dashboard.html','index.html','menu.html','minhas_tarefas.html','calendario.html','pessoal.html','missao.html','ferias_dispensas.html','central.html','usuarios.html','relatorios.html','orcamentarios.html','historico_auditoria.html','configuracoes.html','help.html','about.html','games.html']){await patch(name,s=>s.replaceAll('mobile-dashboard-v184.js','mobile-dashboard-v184.js?v=2.4.7.3-b285'),{required:false})}
await patch('mobile-dashboard-v184.js',s=>{
  const start=s.indexOf('  function ensureCard(){');
  const end=s.indexOf('\n\n  function setCard',start);
  if(start<0||end<0)throw new Error('2.4.7.3: ensureCard do dashboard mobile não encontrado');
