...
node --check scripts/verify-mobile-v243.mjs
...
sed -i 's/versionCode 1/versionCode 278/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.4.3"/' android/app/build.gradle
...
git add downloads/TAREFAS-2.4.3*
if ! git diff --cached --quiet; then
  git commit -m 'release(android): TAREFAS 2.4.3 beta build 278'
  git push origin HEAD:app/releases
fi
cd "$GITHUB_WORKSPACE"

URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.4.3.apk'
...
jq -n \
  ...
  --arg title 'TAREFAS 2.4.3 Beta — Seletor de ícones' \
  --arg url "$URL" \
  --arg sha "$SHA" \
  '{
    version:$version,
...
test "$CODE" = '200'
jq -e '.ok == true and .version == "2.4.3" and .build == 278 and .channel == "beta"' "$RUNNER_TEMP/result.json" >/dev/null
