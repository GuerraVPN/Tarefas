...
const mustFile=async(f,label)=>{try{await access(f)}catch{throw new Error('2.4.7.5: arquivo ausente: '+label)}};
...
if(patchData.id!==PATCH_ID||patchData.baseVersion!=='2.4.6'||!patchData.payload?.js)throw new Error('2.4.7.5: tpatch 2.4.6.8 inválido');
if(String(patchData.payloadSha256||'').toLowerCase()!==PATCH_SHA256)throw new Error('2.4.7.5: SHA-256 do tpatch 2.4.6.8 não confere');
if(JSON.stringify(patchData).includes('__TAREFAS_ALPHA_2467_ESCALAS_2433__'))throw new Error('2.4.7.5: conteúdo de 2.4.6.7 detectado');
...
await patch('mobile-login-v17.js',s=>s.replaceAll("location.replace('dashboard.html')","location.replace('dashboard.html?app=2.4.7.5')"));
await patch('index.html',s=>s.replaceAll('window.location.replace("dashboard.html")','window.location.replace("dashboard.html?app=2.4.7.5")').replaceAll('window.location.href = "dashboard.html"','window.location.href = "dashboard.html?app=2.4.7.5"'));
await patch('mobile-bootstrap.js',s=>s.replaceAll("['home','dashboard.html','Início'","['home','dashboard.html?app=2.4.7.5','Início'"));
...
await writeFile(path.join(dist,'ALPHA_2_4_7_5.json'),JSON.stringify({
 ...
 features:{only2468:true,scalesPatch2467:false,servicesHotbar2468:true,dashboardPathRebuilt:true,legacyDashboardModuleRemoved:true,cacheBustedDashboardEntry:true}
},null,2)+'\n');
