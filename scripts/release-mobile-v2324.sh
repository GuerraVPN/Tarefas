...
node --check app/mobile-patch-manager-v2324.js
node --check scripts/build-mobile-v2324.mjs
node --check scripts/verify-mobile-v2324.mjs
node scripts/build-mobile-v2324.mjs
node scripts/verify-mobile-v2324.mjs dist
...
sed -i 's/versionCode 1/versionCode 271/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.3.24"/' android/app/build.gradle
...
cp app/mobile-alpha-v23223-fix.js app/mobile-patch-manager-v2324.js app/release-v2324.txt "$RUNNER_TEMP/package/app/"
cp scripts/build-mobile-v2324.mjs scripts/verify-mobile-v2324.mjs "$RUNNER_TEMP/package/scripts/"
cp dist/BETA_2_3_24.json "$RUNNER_TEMP/package/manifest/"
...
git add downloads/TAREFAS-2.3.24*
if ! git diff --cached --quiet; then git commit -m 'release(android): TAREFAS 2.3.24 beta build 271'; git push origin HEAD:app/releases; fi
cd "$GITHUB_WORKSPACE"
URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.3.24.apk'
...
jq -n --arg version "$VERSION" --argjson build "$BUILD" --arg channel 'beta' --arg web_version '7.8.6' --arg title 'TAREFAS 2.3.24 Beta — Patches consolidados' --arg url "$URL" --arg sha "$SHA" '{version:$version,build:$build,channel:$channel,web_version:$web_version,title:$title,changelog:["🩹 Sistema de patches consolidado nativamente no APK Beta 2.3.24.","♻️ Patches cumulativos substituem automaticamente versões anteriores da mesma base.","📜 Gerenciador exibe histórico e changelog dos patches.","🔔 Notificações de patches respeitam os canais Alpha/Beta e evitam aviso depois da instalação.","🌐 Catálogo e arquivos oficiais usam rota anti-cache pela API do GitHub, com fallback seguro.","🏷️ A versão efetiva dos patches aparece no cabeçalho de todas as abas.","🎨 Corrigida a persistência do tema para não voltar sozinho ao modo claro.","📦 Consolidadas as correções dos patches 2.3.23.2 até 2.3.23.6.","🧱 Nova base de patches: 2.3.24 / build 271."],mandatory:false,download_url:$url,sha256:$sha}' > "$RUNNER_TEMP/release.json"
