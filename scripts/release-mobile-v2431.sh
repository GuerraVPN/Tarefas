...
node --check scripts/build-mobile-v2431.mjs
node --check scripts/verify-mobile-v2431.mjs
node scripts/verify-web.mjs .

# Recria exatamente a Oficial 2.4.0 e aplica somente a camada Beta 2.4.3.
git fetch origin beta-2.3.25
BASE_DIR="$RUNNER_TEMP/base-2325-v243"
...
node scripts/build-mobile-v2431.mjs
node scripts/verify-mobile-v2431.mjs dist
...
sed -i 's/versionName "1.0"/versionName "2.4.3.1"/' android/app/build.gradle
...
cp app/mobile-launcher-icon-v241.js app/release-v2431.txt "$RUNNER_TEMP/package/app/"
cp app/android/TarefasLauncherIconPlugin.java "$RUNNER_TEMP/package/app/"
cp scripts/build-mobile-v2431.mjs scripts/verify-mobile-v2431.mjs "$RUNNER_TEMP/package/scripts/"
cp dist/ALPHA_2_4_3_1.json "$RUNNER_TEMP/package/manifest/"
...
git add downloads/TAREFAS-2.4.3.1*
if ! git diff --cached --quiet; then
  git commit -m 'release(android): TAREFAS 2.4.3.1 alpha build 279'
  git push origin HEAD:app/releases
fi
cd "$GITHUB_WORKSPACE"

URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.4.3.1.apk'
...
jq -n \
  ...
  --arg title 'TAREFAS 2.4.3.1 Alpha — integração de escalas' \
  --arg url "$URL" \
  --arg sha "$SHA" \
  '{
    version:$version,
...
test "$CODE" = '200'
jq -e '.ok == true and .version == "2.4.3.1" and .build == 279 and .channel == "alpha"' "$RUNNER_TEMP/result.json" >/dev/null
