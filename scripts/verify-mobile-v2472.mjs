...
const must=(x,m)=>{if(!x)throw new Error('2.4.7.2 verify: '+m)};
...
must(b.includes("const APP_VERSION = '2.4.7.2';")&&b.includes('const APP_BUILD = 284;'),'versão/build do bootstrap incorretos');
must(pm.includes("APP_VERSION='2.4.7.2',APP_BUILD=284,APP_CHANNEL='alpha'"),'Patch Manager alpha incorreto');
must(u.includes("const APP_VERSION = '2.4.7.2';")&&u.includes('const APP_BUILD = 284;')&&(u.includes("const APP_CHANNEL = 'alpha';")||u.includes("const APP_CHANNEL='alpha';")),'updates incorreto');
...
must(man.version==='2.4.7.2'&&man.build===284&&man.channel==='alpha'&&man.base==='2.4.6'&&man.basedOn==='2.4.7.1'&&man.incorporatedPatch==='2.4.6.8','manifesto incorreto');
...
console.log('VERIFY 2.4.7.2 ALPHA OK: somente patch 2.4.6.8 + cartão Próximo Serviço removido.');
