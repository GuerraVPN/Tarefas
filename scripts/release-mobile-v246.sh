...
sed -i 's/versionName "1.0"/versionName "2.4.6"/' android/app/build.gradle
...
git add downloads/TAREFAS-2.4.6*
if ! git diff --cached --quiet; then
  git commit -m 'release(android): TAREFAS 2.4.6 beta build 281'
  git push origin HEAD:app/releases
fi
cd "$GITHUB_WORKSPACE"

URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.4.6.apk'
