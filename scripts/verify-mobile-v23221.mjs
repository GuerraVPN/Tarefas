...
for(const m of ["const APP_VERSION = '2.3.22.1';",'const APP_BUILD = 264;','__TAREFAS_ALPHA_23221__',"channel:'alpha'",'Novidades da 2.3.22.1 Alpha','Central de Notificações 2.0','Fila offline e sincronização','IA com acesso às ferramentas da Alpha'])need(bootstrap,m,'mobile-bootstrap.js');
...
for(const name of htmlFiles){const h=await read(name);need(h,'mobile-alpha-v23221-core.js?v=2.3.22.1-b264',name);need(h,'mobile-alpha-v23221.js?v=2.3.22.1-b264',name)}
const manifest=JSON.parse(await read('ALPHA_2_3_22_1.json'));if(manifest.version!=='2.3.22.1'||manifest.build!==264||manifest.channel!=='alpha')throw new Error('Manifesto Alpha inválido');if(!manifest.features?.notifications2||!manifest.features?.aiLocalCommands||!manifest.features?.destructiveConfirmation)throw new Error('Manifesto sem recursos obrigatórios');
console.log(`CONFERÊNCIA 1/3 OK: ${htmlFiles.length} telas, versão/build/canal, regressões 2.3.21.x, Central 2.0, offline e IA validados.`);
