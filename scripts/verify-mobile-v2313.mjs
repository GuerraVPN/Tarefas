...
for(const r of ['mobile-bootstrap.js','mobile-preload.js','native-mobile.js','aditamento_v74.js','pessoal.html','jspdf.umd.min.js','orcamentarios.html','orcamentarios-loader-v242.js','pedidos_v6.js','movimentacoes_v6.js','v7_7_0_material_carga.js'])if(!await exists(r))errors.push(`arquivo ausente: ${r}`);
...
forbid(java,'intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);\n                getContext().startActivity(fallback);','StorageAccessPlugin fallback');
...
if(await exists('orcamentarios.html')){const s=await text('orcamentarios.html');req(s,'orcamentarios-loader-v242.js?v=2.3.13-b243','orcamentarios')}
...
if(await exists('v7_7_0_material_carga.js')){const s=await text('v7_7_0_material_carga.js');req(s,'__TAREFAS_MATERIAL_CARGA_EVENT_DRIVEN_V242__','material carga');for(const m of ['MutationObserver','setInterval('])forbid(s,m,'material carga')}

for(const r of ['aditamento_v74.js','orcamentarios-loader-v242.js','v7_7_0_material_carga.js','material_carga_v6.js','pedidos_v6.js','movimentacoes_v6.js','native-mobile.js'])if(await exists(r)){
...
console.log('OK: 2.3.13 build 243 — seletor nativo Salvar como ligado aos arquivos gerados; Aditamento PDF/ODT e proteções do Orçamentários preservados.');