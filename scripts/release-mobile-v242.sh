...
node --check scripts/build-mobile-v242.mjs
node --check scripts/verify-mobile-v242.mjs
node scripts/verify-web.mjs .

# Recria exatamente a Oficial 2.4.0 e aplica somente a camada Beta 2.4.2.
git fetch origin beta-2.3.25
BASE_DIR="$RUNNER_TEMP/base-2325-v242"
...
node scripts/verify-mobile-v242.mjs dist
...
sed -i 's/versionCode 1/versionCode 277/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.4.2"/' android/app/build.gradle
...
assert int(e['versionCode']) == 277, e
...
grep -q "v1_8_get_beta_updates" "$RUNNER_TEMP/pm-v242.js"
grep -q "v2_3_21_alpha_context" "$RUNNER_TEMP/pm-v242.js"
grep -q "const APP_CHANNEL = 'beta';" "$RUNNER_TEMP/updates-v242.js"
...
RELEASES_DIR="$RUNNER_TEMP/tarefas-releases-v242"
...
git add downloads/TAREFAS-2.4.2*
if ! git diff --cached --quiet; then
  git commit -m 'release(android): TAREFAS 2.4.2 beta build 277'
  git push origin HEAD:app/releases
fi
cd "$GITHUB_WORKSPACE"

URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.4.2.apk'
...
jq -n \
  ...
  --arg title 'TAREFAS 2.4.2 Beta — Seletor de ícones' \
  --arg url "$URL" \
  --arg sha "$SHA" \
  '{
    version:$version,
...
test "$CODE" = '200'
jq -e '.ok == true and .version == "2.4.2" and .build == 277 and .channel == "beta"' "$RUNNER_TEMP/result.json" >/dev/null
