...
node --check scripts/build-mobile-v241.mjs
node --check scripts/verify-mobile-v241.mjs
node scripts/verify-web.mjs .

# Recria exatamente a Oficial 2.4.0 e aplica somente a camada Beta 2.4.1.
git fetch origin beta-2.3.25
BASE_DIR="$RUNNER_TEMP/base-2325-v241"
...
node scripts/build-mobile-v241.mjs
node scripts/verify-mobile-v241.mjs dist
...
sed -i 's/versionCode 1/versionCode 276/' android/app/build.gradle
sed -i 's/versionName "1.0"/versionName "2.4.1"/' android/app/build.gradle
...
assert int(e['versionCode']) == 276, e
assert str(e['versionName']) == '2.4.1', e
PY

unzip -p "$UNSIGNED" assets/public/mobile-launcher-icon-v241.js > "$RUNNER_TEMP/launcher-v241.js"
unzip -p "$UNSIGNED" assets/public/mobile-bootstrap.js > "$RUNNER_TEMP/bootstrap-v241.js"
unzip -p "$UNSIGNED" assets/public/mobile-patch-manager-v240.js > "$RUNNER_TEMP/pm-v241.js"
unzip -p "$UNSIGNED" assets/public/mobile-updates-v181.js > "$RUNNER_TEMP/updates-v241.js"
grep -q '__TAREFAS_LAUNCHER_ICON_241__' "$RUNNER_TEMP/launcher-v241.js"
grep -q 'pinProfileShortcut' "$RUNNER_TEMP/launcher-v241.js"
grep -q '__TAREFAS_BETA_241_BOOT__' "$RUNNER_TEMP/bootstrap-v241.js"
grep -q '__TAREFAS_PATCH_MANAGER_V276__' "$RUNNER_TEMP/pm-v241.js"
grep -q "v1_8_get_beta_updates" "$RUNNER_TEMP/pm-v241.js"
grep -q "v2_3_21_alpha_context" "$RUNNER_TEMP/pm-v241.js"
grep -q "const APP_CHANNEL = 'beta';" "$RUNNER_TEMP/updates-v241.js"
...
cp app/mobile-launcher-icon-v241.js app/release-v241.txt "$RUNNER_TEMP/package/app/"
cp app/android/TarefasLauncherIconPlugin.java "$RUNNER_TEMP/package/app/"
cp scripts/build-mobile-v241.mjs scripts/verify-mobile-v241.mjs "$RUNNER_TEMP/package/scripts/"
...
RELEASES_DIR="$RUNNER_TEMP/tarefas-releases-v241"
...
git add downloads/TAREFAS-2.4.1*
if ! git diff --cached --quiet; then
  git commit -m 'release(android): TAREFAS 2.4.1 beta build 276'
  git push origin HEAD:app/releases
fi
cd "$GITHUB_WORKSPACE"

URL='https://raw.githubusercontent.com/GuerraVPN/Tarefas/app/releases/downloads/TAREFAS-2.4.1.apk'
...
jq -n \
  ...
  --arg title 'TAREFAS 2.4.1 Beta — Seletor de ícones' \
  --arg url "$URL" \
  --arg sha "$SHA" \
  '{
    version:$version,
...
test "$CODE" = '200'
jq -e '.ok == true and .version == "2.4.1" and .build == 276 and .channel == "beta"' "$RUNNER_TEMP/result.json" >/dev/null
