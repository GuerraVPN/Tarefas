...
let user=null,isAdmin=false,isFiscal=false,isCommander=false;
...
const MOV_STATUS={
 ...
 aguardando_aprovacao_detentor:'Aguardando aprovação do detentor da carga',
 aguardando_aprovacao_cmt:'Aguardando aprovação do Cmt',
 aguardando_assinatura_detentor:'Aguardando assinatura do detentor da carga',
 aguardando_assinatura_cmt:'Aguardando assinatura do Cmt',
 encaminhado_base:'Encaminhado à Base',
 pronto:'Pronto'
};
function movStatusClass(s){
 if(s==='retornado_fiscal')return'return';
 if(s==='pronto')return'ok';
 if(['aguardando_aprovacao_detentor','aguardando_aprovacao_cmt','aguardando_assinatura_detentor','aguardando_assinatura_cmt'].includes(s))return'info';
 return'wait';
}
...
function renderDetail(){
 ...
 $('mdStatus').textContent=MOV_STATUS[selected.status]||selected.status||'Encaminhado ao Fiscal';
 $('mdStatus').className=`pedido-status ${movStatusClass(selected.status)}`;
 const returned=selected.status==='retornado_fiscal';
 $('movReturnNotice').hidden=!returned;
 $('movReturnNotice').textContent=returned?`⚠️ Motivo do retorno: ${selected.retorno_motivo||'Não informado.'}`:'';
 renderMovFlow();renderMovActions();
...
function renderHistory(){
 ...
 $('mdHistory').innerHTML=history.length?history.map(h=>`<div class="pedido-hitem"><strong>${esc(({movimentacao_criada:'Movimentação cadastrada e encaminhada à Fiscalização',
    movimentacao_editada:'Movimentação editada',arquivo_adicionado:'Arquivo anexado',atualizacao:'Atualização',
    mov_aprovada_fiscal:'Fiscalização aprovou a movimentação',
    mov_retornada_fiscal:'Fiscalização retornou a movimentação',
    mov_reenviada_fiscal:'Movimentação reenviada à Fiscalização',
    mov_aprovada_detentor:'Detentor da carga aprovou',
    mov_aprovada_cmt:'Cmt aprovou',
    mov_assinada_detentor:'Assinatura do detentor registrada',
    mov_assinada_cmt_encaminhada_base:'Assinatura do Cmt registrada / Encaminhada à Base',
    mov_concluida:'Movimentação concluída'}[h.evento]||h.evento))}</strong>${h.mensagem?`<p>${esc(h.mensagem)}</p>`:''}<small>${esc(userName(h.usuario_id))} · ${dtBR(h.criado_em)}</small></div>`).join(''):'<div class="orc-empty">Sem histórico.</div>';
}
function movFlowRank(s){
 return ({
  encaminhado_fiscal:1,retornado_fiscal:1,
  aguardando_aprovacao_detentor:2,
  aguardando_aprovacao_cmt:3,
  aguardando_assinatura_detentor:4,
  aguardando_assinatura_cmt:5,
  encaminhado_base:6,
  pronto:7
 }[s]??0);
}
function renderMovFlow(){
 const labels=[
  'Encaminhado ao Fiscal','Aprovado / Retornado',
  'Aprovação do detentor','Aprovação do Cmt',
  'Assinatura do detentor','Assinatura do Cmt',
  'Encaminhado à Base','Pronto'
 ];
 const r=movFlowRank(selected.status);
 $('movFlowHint').textContent=MOV_STATUS[selected.status]||selected.status||'';
 $('movSteps').innerHTML=labels.map((t,i)=>`<div class="pedido-step ${i<r||selected.status==='pronto'?'done':''} ${i===r?'current':''}">${esc(t)}</div>`).join('');
}
function movBtn(label,action,cls=''){return `<button class="orc-btn ${cls}" data-mov-action="${action}">${label}</button>`}
function renderMovActions(){
 let h='',hint='',s=selected.status;
 if(s==='encaminhado_fiscal'){
   hint='Aguardando Fiscalização';
   h=isFiscal
     ?movBtn('↩ Retornar','retornar_fiscal','danger')+movBtn('✓ Aprovar','aprovar_fiscal','primary')
   ...
   h=canManage(selected)?movBtn('Reencaminhar ao Fiscal','reenviar_fiscal','primary'):'';
 }else if(s==='aguardando_aprovacao_detentor'){
   hint='Aguardando aprovação do detentor da carga';
   h=canManage(selected)?movBtn('✓ Registrar aprovação do detentor','aprovar_detentor','primary'):'';
 }else if(s==='aguardando_aprovacao_cmt'){
   hint='Aguardando aprovação do Cmt';
   h=isCommander?movBtn('✓ Registrar aprovação do Cmt','aprovar_cmt','primary'):'<span class="pedido-status wait">Aguardando perfil Comandante</span>';
 }else if(s==='aguardando_assinatura_detentor'){
   hint='Aguardando assinatura do detentor da carga';
   h=canManage(selected)?movBtn('✍️ Registrar assinatura do detentor','assinar_detentor','primary'):'';
 }else if(s==='aguardando_assinatura_cmt'){
   hint='Aguardando assinatura do Cmt';
   h=isCommander?movBtn('✍️ Assinatura do Cmt · Encaminhar à Base','assinar_cmt','primary'):'<span class="pedido-status wait">Aguardando perfil Comandante</span>';
 }else if(s==='encaminhado_base'){
   hint='Encaminhado à Base';
   h=canManage(selected)?movBtn('✓ Marcar como Pronto','pronto','primary'):'';
 }else{
   hint='Concluído';
   h='<span class="pedido-status ok">✓ Pronto</span>';
 }
 $('movActionHint').textContent=hint;
 $('movActionButtons').innerHTML=h;
}
async function moveMov(action){
 if(!selected)return;
 const note=$('movActionNote').value.trim();
 if(action==='retornar_fiscal'&&!note)return alert('Informe o motivo do retorno.');

 const r=await supabaseClient.rpc('v5_4_2_mover_movimentacao',{
   p_movimentacao_id:selected.id,
   p_acao:action,
 ...
 await loadAll();await selectMov(id);
}
...
async function saveMov(e){
 ...
 try{
   ...
   await supabaseClient.from('movimentacao_material_tramitacoes').insert([{movimentacao_id:mov.id,evento:'movimentacao_criada',mensagem:'Movimentação cadastrada e encaminhada à Fiscalização.',usuario_id:String(user.id),perfil_id:profileId()}]);
...
function switchToMov(){
 ...
 if($('reportModule'))$('reportModule').hidden=true;$('guiasModule').hidden=true;$('pedidosModule').hidden=true;if($('materialCargaModule'))$('materialCargaModule').hidden=true;if($('passagemCargaModule'))$('passagemCargaModule').hidden=true;$('movimentacaoModule').hidden=false;
 $('orcPageTitle').textContent='Orçamentários · Movimentação de Material';
 $('orcPageSubtitle').textContent='Fiscalização, aprovações, assinaturas, Base, materiais, anexos e atualizações.';
 renderList();
}
function bind(){
