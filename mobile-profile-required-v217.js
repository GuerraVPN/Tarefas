(function(){
'use strict';
if(window.__TAREFAS_PROFILE_REQUIRED_V217__)return;
window.__TAREFAS_PROFILE_REQUIRED_V217__=true;
...
function style(){
 if(document.getElementById('profileRequiredV217Style'))return;
 const s=document.createElement('style');s.id='profileRequiredV217Style';s.textContent=`
 #profileRequiredV217{position:fixed;inset:0;z-index:2147483646;background:rgba(4,8,8,.88);backdrop-filter:blur(7px);display:flex;align-items:center;justify-content:center;padding:18px;font-family:Inter,Arial,sans-serif}
 #profileRequiredV217 .pr-card{width:min(520px,100%);max-height:calc(100vh - 36px);overflow:auto;background:var(--v4-surface,#fff);color:var(--v4-text,#111827);border:1px solid var(--v4-border,#d1d5db);border-radius:18px;box-shadow:0 24px 80px rgba(0,0,0,.45);padding:22px}
 #profileRequiredV217 .pr-icon{width:52px;height:52px;border-radius:15px;background:#f59e0b;color:#111827;display:grid;place-items:center;font-size:27px;margin-bottom:14px}
 #profileRequiredV217 h2{margin:0 0 8px;font-size:21px}#profileRequiredV217 p{margin:0 0 14px;color:var(--v4-muted,#6b7280);font-size:13px;line-height:1.5}
 #profileRequiredV217 .pr-pending{display:flex;gap:7px;flex-wrap:wrap;margin:0 0 15px}#profileRequiredV217 .pr-chip{padding:6px 9px;border-radius:999px;background:#fff7ed;color:#9a3412;border:1px solid #fed7aa;font-size:11px;font-weight:700}
 #profileRequiredV217 label{display:block;font-size:12px;font-weight:700;margin:12px 0 5px}#profileRequiredV217 input{width:100%;box-sizing:border-box;padding:12px;border-radius:9px;border:1px solid var(--v4-border-strong,#cbd5e1);background:var(--v4-surface,#fff);color:var(--v4-text,#111827);font-size:15px;outline:none}
 #profileRequiredV217 input:focus{border-color:#047857;box-shadow:0 0 0 3px rgba(4,120,87,.12)}#profileRequiredV217 .pr-msg{min-height:18px;margin-top:10px;color:#b91c1c;font-size:12px;font-weight:600}
 #profileRequiredV217 button{width:100%;margin-top:8px;border:0;border-radius:10px;padding:12px 14px;background:#047857;color:#fff;font-size:14px;font-weight:800;cursor:pointer}#profileRequiredV217 button:disabled{opacity:.55;cursor:wait}
 #profileRequiredV217 .pr-foot{font-size:10px;color:var(--v4-muted,#6b7280);text-align:center;margin-top:12px}
...
function render(row,missing){
 if(overlay)return;
 style();
 overlay=document.createElement('div');overlay.id='profileRequiredV217';
 overlay.innerHTML=`<div class="pr-card" role="dialog" aria-modal="true" aria-labelledby="prTitle">
   ...
   <form id="profileRequiredV217Form" novalidate>
   ...
   </form><div class="pr-foot">Beta 2.1.7 • seus dados serão conferidos no servidor antes de liberar o app.</div>
  </div>`;
 ...
 form.addEventListener('submit',async e=>{
  ...
  try{
   const c=await waitClient();const u=loggedUser();if(!c||!u?.id)throw new Error('Sessão indisponível. Entre novamente no aplicativo.');
 ...
 // Impede sair da etapa obrigatória pelo botão voltar enquanto houver pendências.
 try{history.pushState({profileRequiredV217:true},'',location.href);window.addEventListener('popstate',()=>{if(overlay)history.pushState({profileRequiredV217:true},'',location.href)})}catch(_){}
}
...
async function check(force=false){
 ...
 }catch(e){console.warn('[TAREFAS 2.1.7] falha ao conferir cadastro',e?.message||e)}finally{checking=false}
}
