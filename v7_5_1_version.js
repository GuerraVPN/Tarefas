...
function apply(){
 ...
 try{
  ...
  document.querySelectorAll('.v65-version-badge').forEach(b=>{if(b.textContent!==badgeText)b.textContent=badgeText;b.title='Sobre a versão '+VERSION;});
  ...
  if(siteTitle&&siteTitle.textContent!==`Painel SITE · V${VERSION}`)siteTitle.textContent=`Painel SITE · V${VERSION}`;
 }finally{busy=false}
}
