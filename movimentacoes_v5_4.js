...
function filtered(){
 ...
 if(q)arr=arr.filter(x=>norm(`${x.numero} ${x.dependencia_origem} ${x.dependencia_destino} ${x.finalidade} ${its(x.id).map(i=>`${i.nome} ${i.numero_ficha}`).join(' ')}`).includes(q));
 return arr;
}
function renderList(){
...
function renderItems(){
 ...
 $('mdItemsBody').innerHTML=selectedItems.map(i=>`<tr><td>${esc(i.nome)}</td><td>${esc(i.numero_ficha)}</td><td class="num">${esc(i.quantidade)}</td><td class="num">${esc(money(i.valor_unitario))}</td><td class="num"><b>${esc(money(i.valor_total))}</b></td></tr>`).join('');
...
function readItems(){return [...document.querySelectorAll('#movItemsEditor .pedido-item-row')].map(r=>({nome:r.querySelector('.mi-name').value.trim(),numero_ficha:r.querySelector('.mi-ficha').value.trim(),quantidade:Number(r.querySelector('.mi-qtd').value),valor_unitario:Number(r.querySelector('.mi-unit').value)}))}
function openNew(){
...
async function saveMov(e){
 ...
 if(its.some(i=>!i.nome||!i.numero_ficha||!(i.quantidade>0)||i.valor_unitario<0))return alert('Confira os materiais.');
