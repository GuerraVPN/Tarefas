(()=>{
'use strict';
const VERSION='7.9.2',MARK='__TAREFAS_SITE_792__',POLL_IDLE=10000,POLL_PENDING=2000;
...
const KEY_EXIT='tarefas_site_792_exit_seen';
const KEY_RESTART='tarefas_site_792_restart_seen';
const KEY_LAST_ACTION='tarefas_site_792_last_action';
...
function injectCss(){
 if($('site792Css'))return;
 const s=document.createElement('style');s.id='site792Css';s.textContent=`
 #site792Warning{position:fixed;left:50%;top:10px;transform:translateX(-50%);z-index:2147483600;max-width:min(820px,calc(100vw - 22px));padding:11px 15px;border-radius:12px;background:#7f1d1d;color:#fff;border:1px solid #ef4444;box-shadow:0 14px 34px #0008;font:800 12px/1.4 Arial,sans-serif;text-align:center}
 #site792Warning.admin{background:#78350f;border-color:#f59e0b}
 #site792Modal{position:fixed;inset:0;z-index:2147483646;background:#000a;display:grid;place-items:center;padding:18px;font-family:Arial,sans-serif}
 #site792Modal[hidden]{display:none!important}#site792Modal .card{width:min(600px,100%);background:#0f172a;color:#e5e7eb;border:1px solid #334155;border-radius:18px;padding:18px;box-shadow:0 25px 70px #000b}
 #site792Modal h3{margin:0 0 5px;color:#fff;font-size:20px}#site792Modal .sub{font-size:11px;color:#94a3b8;margin-bottom:14px}
 #site792Modal .status{padding:11px;border:1px solid #334155;border-radius:10px;background:#111827;margin-bottom:12px;font-size:12px;line-height:1.55}
 #site792Modal .grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}#site792Modal button{border:1px solid #475569;background:#111827;color:#e5e7eb;border-radius:10px;padding:11px;font-weight:800;cursor:pointer}
 #site792Modal button.danger{border-color:#ef4444;color:#fecaca}#site792Modal button.warn{border-color:#f59e0b;color:#fde68a}#site792Modal button.good{border-color:#22c55e;color:#bbf7d0}#site792Modal .close{width:100%;margin-top:9px}
 #site792Menu .site-dot{width:7px;height:7px;border-radius:50%;margin-left:auto;background:#22c55e;box-shadow:0 0 0 3px rgba(34,197,94,.14)}#site792Menu[data-state="pending"] .site-dot{background:#f59e0b}#site792Menu[data-state="off"] .site-dot{background:#ef4444}
 @media(max-width:520px){#site792Modal .grid{grid-template-columns:1fr}}
...
function ensureMenu(){
 ...
 if(!li){li=document.createElement('li');li.id='site792Menu';li.dataset.v6Nav='site';li.innerHTML='<button type="button" class="v6-nav-item" title="Abrir Painel SITE"><span class="v6-nav-icon">⚙</span><span class="v6-nav-label">Painel SITE</span><span class="site-dot"></span></button>';li.querySelector('button').onclick=openPanel}
...
function renderWarning(){
 let el=$('site792Warning');
 if(!current?.acao_pendente){el?.remove();if(countdown){clearInterval(countdown);countdown=null}return}
 if(!el){el=document.createElement('div');el.id='site792Warning';document.body.appendChild(el)}
...
function executeClientAction(action){
 if(isAdmin)return;
 if(action==='reiniciar'){const done=Number(localStorage.getItem('tarefas_site_792_restart_completed_at')||0);if(done&&Date.now()-done<60000)return}
...
function renderPanelIfOpen(){const m=$('site792Modal');if(!m||m.hidden)return;renderPanel()}
function renderPanel(){
 const m=$('site792Modal');if(!m)return;m.innerHTML=panelHtml();
...
async function openPanel(){
 ...
 let m=$('site792Modal');if(!m){m=document.createElement('div');m.id='site792Modal';document.body.appendChild(m)}m.hidden=false;renderPanel();
}
...
async function init(){
 ...
 try{await loadState()}catch(e){console.warn('[SITE 7.9.2]',e?.message||e);schedulePoll()}
...
window.SitePanel792=Object.freeze({version:VERSION,open:openPanel,refresh:()=>loadState(),state:()=>current,isAdmin:()=>isAdmin,action:requestAction});
