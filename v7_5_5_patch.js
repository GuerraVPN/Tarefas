...
function serviceNav(){
 ...
 document.addEventListener('click',e=>{
  const shift=e.target.closest?.('[data-v743-shift]');if(shift){e.preventDefault();e.stopImmediatePropagation();shiftService(Number(shift.dataset.v743Shift)||0);return}
  const btn=e.target.closest?.('button');if(!btn)return;
  if(btn.id==='prevMonth'||btn.id==='nextMonth'){e.preventDefault();e.stopImmediatePropagation();shiftService(btn.id==='prevMonth'?-1:1);return}
  if(btn.id==='todayMonth'){e.preventDefault();e.stopImmediatePropagation();todayService()}
 },true);
}
function missionNav(){
 if(page!=='missao.html')return;
 document.getElementById('v743Period')?.remove();
 const style=document.createElement('style');style.id='v755MissionPeriodFix';style.textContent='html body .v7-head .v7-month{display:flex!important}';document.head.appendChild(style);
 const clean=()=>document.getElementById('v743Period')?.remove();
 setTimeout(clean,0);setTimeout(clean,300);setTimeout(clean,1000);
}
