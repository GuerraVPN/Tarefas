(()=>{
'use strict';
const VERSION='7.9.1',MARK='__TAREFAS_WEB_791__';
...
function installCss(){
 if(document.getElementById('web791Style'))return;
 const s=document.createElement('style');s.id='web791Style';s.textContent=`
...
function installSafeApi(){
 if(window.TarefasWeb791Safe)return;
 const get=id=>document.getElementById(id);
 window.TarefasWeb791Safe=Object.freeze({
  get,
...
function installLifecycle(){
 if(window.TarefasWeb791Lifecycle)return;
 ...
 window.TarefasWeb791Lifecycle=Object.freeze({token:()=>generation,isActive:t=>active&&t===generation,guard:async(p,t=generation)=>{const v=await p;if(!active||t!==generation){const e=new Error('Resposta assíncrona obsoleta descartada');e.code='TAREFAS_STALE_ASYNC';throw e}return v}});
}
...
async function renderNotif791(){
 const aux=central.aux;if(!aux)return;const life=window.TarefasWeb791Lifecycle,token=life?.token?.();const st=state();const fetched=await fetchNotifs();if(life&&token!=null&&!life.isActive(token))return;central.rows=fetched;const s=st;const f=s.lastNotificationFilter||'todas';const rows=central.rows.filter(n=>f==='todas'||(f==='nao_lidas'?!n.lida:catOf(n)===f));
 ...
 for(const n of rows){const id=String(n.id),d=document.createElement('article');d.className='w790-notif'+(n.lida?'':' unread');d.innerHTML='<input type="checkbox" '+(central.selected.has(id)?'checked':'')+'><div><strong>'+((s.pinnedNotifications||[]).includes(id)?'📌 ':'')+esc(n.titulo||'Notificação')+'</strong><p>'+esc(n.mensagem||'')+'</p><span class="w790-pill">'+esc(catOf(n))+'</span></div><time>'+esc(new Date(n.criada_em).toLocaleString('pt-BR'))+'</time>';d.querySelector('input').onchange=e=>e.target.checked?central.selected.add(id):central.selected.delete(id);d.querySelector('strong').onclick=()=>{s.pinnedNotifications=(s.pinnedNotifications||[]).filter(x=>x!==id);if(!(state().pinnedNotifications||[]).includes(id))s.pinnedNotifications.push(id);writeState(s);renderNotif791()};list.appendChild(d)}
 aux.querySelector('[data-refresh]').onclick=()=>renderNotif791();
 aux.querySelector('[data-filter]').onchange=e=>{const x=state();x.lastNotificationFilter=e.target.value;writeState(x);renderNotif791()};
 aux.querySelector('[data-select]').onclick=()=>{rows.forEach(x=>central.selected.add(String(x.id)));renderNotif791()};
 aux.querySelector('[data-read]').onclick=async()=>{await mutateNotifications('read',[...central.selected]);central.selected.clear();await renderNotif791()};
 aux.querySelector('[data-delete]').onclick=async()=>{if(central.selected.size&&confirm('Excluir notificações selecionadas?')){await mutateNotifications('delete',[...central.selected]);central.selected.clear();await renderNotif791()}};
 aux.querySelector('[data-ret]').onchange=async e=>{const x=state();x.retentionDays=Number(e.target.value)||0;writeState(x);if(x.retentionDays){const cut=Date.now()-x.retentionDays*86400000;const ids=central.rows.filter(n=>n.lida&&!x.pinnedNotifications.includes(String(n.id))&&Date.parse(n.criada_em)<cut).map(n=>n.id);if(ids.length)await mutateNotifications('delete',ids)}await renderNotif791()};
 aux.querySelector('[data-mute]').onclick=()=>{const x=state(),cat=['todas','nao_lidas'].includes(f)?'versoes':f,on=!x.mutedCategories.includes(cat);x.mutedCategories=x.mutedCategories.filter(v=>v!==cat);if(on)x.mutedCategories.push(cat);writeState(x);renderNotif791()};
}
...
function renderFavorites(){
 const s=state();central.aux.innerHTML='<div class="w790-head"><div><h3>⭐ Favoritos</h3><small>Páginas, módulos e subabas · URLs duplicadas são consolidadas automaticamente.</small></div><button class="w790-btn primary" data-pick>Escolher telas e abas</button></div><div class="w790-body">'+(s.favorites.length?s.favorites.map(x=>'<div class="w790-row"><span><strong>'+esc(x.label)+'</strong><small>'+esc(x.href)+'</small></span><div class="w790-actions"><button class="w790-btn" data-open="'+esc(x.href)+'">Abrir</button><button class="w790-btn" data-favrename="'+esc(x.id)+'">Renomear</button><button class="w790-btn danger" data-rm="'+esc(x.id)+'">Excluir</button></div></div>').join(''):'<div class="w790-empty">Nenhum favorito ainda.</div>')+'</div>';
...
async function showCentral(tab){
 ...
 try{if(central.tab==='notificacoes')await renderNotif791();if(central.tab==='downloads')renderDownloads();if(central.tab==='favoritos')renderFavorites();if(central.tab==='ferramentas')renderTools()}catch(e){logError({source:'central',message:e.message});central.aux.innerHTML='<div class="w790-empty">Falha ao carregar: '+esc(e.message)+'</div>'}
}
