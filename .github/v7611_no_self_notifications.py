...
# Toda edição direta de tarefa informa ao Supabase quem executou a ação.
# O trigger central V7.6.11 usa estes campos para não gerar notificação ao próprio autor.
...
replace('v7_5_1_version.js',"const VERSION='7.6.10',BADGE='● TAREFAS v'+VERSION;",f"const VERSION='{VERSION}',BADGE='● TAREFAS v'+VERSION;")
replace('v7_5_1_about.js',"const VERSION='7.6.10';",f"const VERSION='{VERSION}';")
...
# Validação estrutural: nenhum update direto de tarefa pode ficar sem identificação do autor.
for name in ['menu.html','minhas_tarefas.html']:
    s=Path(name).read_text(encoding='utf-8')
    chunks=s.split(needle)[1:]
    if chunks:
        raise SystemExit(f'{name}: ainda existe update de tarefa sem autor V7.6.11')
    if s.count('atualizado_por:Number(usuarioLogado.id)') < 1:
        raise SystemExit(f'{name}: marcador do autor não foi aplicado')

print(f'Web {VERSION}: autor das ações identificado e auto-notificações bloqueadas pelo backend em {len(htmls)} páginas.')
