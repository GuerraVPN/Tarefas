...
async function loadDetentores(){
 try{
   const [d,u,p]=await Promise.all([
     ...
     supabaseClient.from('usuarios').select('id,patente,nome_guerra'),
     supabaseClient.from('usuario_perfis').select('id,secao,posicao')
   ]);
   detentores=d.error?[]:(d.data||[]);users=new Map((u.error?[]:(u.data||[])).map(x=>[String(x.id),x]));profiles=new Map((p.error?[]:(p.data||[])).map(x=>[String(x.id),x]));
 }catch(_){detentores=[]}
}
...
function renderDetail(){
 ...
 $('cargaDetentorPanel').hidden=tipo!=='dependencia';
 if(tipo==='dependencia'){
   $('cargaDetentorNome').textContent=h?uname(h.usuario_id):'Não definido';
   const pf=h?.perfil_id?profiles.get(String(h.perfil_id)):null;
   $('cargaDetentorPerfil').textContent=pf?[pf.secao,pf.posicao].filter(Boolean).join(' — '):'';
   $('cargaDetentorDesde').textContent=h?.desde?`Desde ${dateBR(h.desde)}`:'Sem detentor cadastrado';
 }
...
async function upload(){
 ...
 try{
   ...
   const r=await supabaseClient.rpc('v5_5_registrar_documento_carga',{
     ...
     p_arquivo_tamanho:file.size,p_usuario_id:String(user.id),p_perfil_id:profileId(),p_observacao:$('cargaNote').value.trim()||null
   });
...
function switchToCarga(){
 ...
 $('guiasModule').hidden=true;$('pedidosModule').hidden=true;$('movimentacaoModule').hidden=true;$('passagemCargaModule').hidden=true;$('materialCargaModule').hidden=false;
 $('orcPageTitle').textContent='Orçamentários · Material Carga / Depósito';
 $('orcPageSubtitle').textContent='Detentor atual, consulta para todos e atualização de documentos somente pela Fiscalização.';
 renderList();
}
...
function bind(){
 ...
 $('btnIrPassagem').onclick=()=>{location.href=`orcamentarios.html?modulo=passagem_carga${selectedRef?'&dependencia='+encodeURIComponent(selectedRef):''}`};
}
